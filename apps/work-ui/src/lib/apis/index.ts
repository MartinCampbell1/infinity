...
import { getFounderosEmbeddedSessionAuthHeaders } from '$lib/founderos/credentials';
...
export const getToolServerData = async (headers: Record<string, string>, url: string) => {
	let error = null;
			...
			servers
				...
				.map(async (server) => {
					let error = null;
						...
						let toolServerHeaders = {};

						const auth_type = server?.auth_type ?? 'bearer';
						if (auth_type === 'bearer') {
							toolServerHeaders = server?.key
								? { authorization: `Bearer ${server?.key}` }
								: {};
						} else if (auth_type === 'none') {
							// No authentication
						} else if (auth_type === 'session') {
							toolServerHeaders = getFounderosEmbeddedSessionAuthHeaders();
						}
