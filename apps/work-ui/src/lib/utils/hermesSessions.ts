...
type HermesAwareSortableChatListItem = HermesAwareChatListItem & {
	__hermes_order_index: number;
};
...
const getSessionStringList = (
	value: string[] | null | undefined
): string[] =>
	Array.isArray(value)
		? value
				.map((item) => normalizeSessionValue(item))
				.filter((item): item is string => Boolean(item))
		: [];

const getSessionOrganizationTerms = (session: HermesSessionListItem) => [
	...getHermesSessionTags(session),
	getHermesSessionGroupLabel(session),
	isHermesSessionPinned(session) ? 'pinned' : null,
	isHermesSessionArchived(session) ? 'archived' : null
].filter((item): item is string => Boolean(item));
	...
	normalizeSearchValue(
		[
			...
			session.last_user_content,
				...
				...getSessionOrganizationTerms(session)
			].join(' ')
		);

export const isHermesSessionPinned = (session: HermesSessionListItem) =>
	session.pinned === true || session.imported_chat_pinned === true;

export const isHermesSessionArchived = (session: HermesSessionListItem) =>
	session.imported_chat_archived === true || session.archived === true;

export const getHermesSessionGroupLabel = (session: HermesSessionListItem) =>
	normalizeSessionValue(session.group_name) ?? normalizeSessionValue(session.group_id) ?? null;

export const getHermesSessionTags = (session: HermesSessionListItem) => {
	const tags = [
		...getSessionStringList(session.tags),
		...getSessionStringList(session.session_tags),
		...getSessionStringList(session.labels)
	];

	return Array.from(new Set(tags.map((tag) => tag.toLowerCase()))).slice(0, 8);
};

export const getHermesSessionOrganizationBadges = (
	session: HermesSessionListItem,
	{ maxTags = 2 }: { maxTags?: number } = {}
) => {
	const badges: string[] = [];
	const tags = getHermesSessionTags(session);
	const groupLabel = getHermesSessionGroupLabel(session);

	if (isHermesSessionPinned(session)) {
		badges.push('Pinned');
	}

	if (groupLabel) {
		badges.push(`Group: ${groupLabel}`);
	}

	for (const tag of tags.slice(0, Math.max(0, maxTags))) {
		badges.push(`#${tag}`);
	}

	if (tags.length > Math.max(0, maxTags)) {
		badges.push(`+${tags.length - Math.max(0, maxTags)} tags`);
	}

	if (isHermesSessionArchived(session)) {
...
export const buildHermesAwareChatList = (
	...
	chats
		.map<HermesAwareSortableChatListItem>((chat, index) => {
