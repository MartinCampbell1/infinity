<script lang="ts">
	...
	export let onImport: (item: ImportItem) => void = () => {};
	export let onClose: () => void = () => {};

	export let loadUrlHandler: (url: string) => Promise<ImportItem | null> = async () => null;
	...
	const submitHandler = async () => {
		...
		const res = await loadUrlHandler(url).catch((err: unknown) => {
			toast.error(`${err}`);
			loading = false;
			return null;
		});
		...
		if (res) {
			...
			const frontmatter = extractFrontmatter(res.content ?? '') as ImportFrontmatter;
