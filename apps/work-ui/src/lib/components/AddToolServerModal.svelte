<script lang="ts">
	...
	export let onSubmit: (connection: ToolServerConnection) => void | Promise<void> = () => {};
	...
	let type: 'openapi' | 'mcp' = 'openapi';

	let url = '';

	let spec_type: 'url' | 'json' = 'url';
	let spec = '';
	...
	let oauthClientInfo: Record<string, unknown> | null = null;
	...
	const submitHandler = async () => {
		...
		const payload: ToolServerConnection = {
			type,
			url,
