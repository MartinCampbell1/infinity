<script lang="ts">
	...
	type FunctionRecord = {
		...
		is_active?: boolean;
		is_global?: boolean;
		user?: FunctionUserRecord | null;
		meta?: {
			description?: string | null;
			manifest?: FunctionManifestRecord | null;
		} | null;
		[key: string]: unknown;
	};

	type FunctionImportPayload = {
		...
		filteredItems = (functions ?? [])
			.filter(
				(f) =>
					...
					(query === '' ||
						f.name.toLowerCase().includes(currentQuery) ||
						f.id.toLowerCase().includes(currentQuery) ||
						(f.user?.name || '').toLowerCase().includes(currentQuery) ||
						(f.user?.email || '').toLowerCase().includes(currentQuery) ||
						(f.user?.username || '').toLowerCase().includes(currentQuery)) &&
					(viewOption === '' ||
	...
	const shareHandler = async (func: FunctionRecord) => {
		...
		if (!tab) {
			toast.error($i18n.t('Unable to open a new tab'));
			return;
		}
...
{#if loaded}
	...
	<DeleteConfirmDialog
		...
		message={
			selectedFunction
				...
				: $i18n.t('This will delete this function.')
		}
		on:confirm={() => {
			if (selectedFunction) {
	...
	<ValvesModal
		...
		type="function"
		id={(selectedFunction?.id ?? null) as unknown as null}
			on:save={async () => {
				await tick();
				models.set(
					await getModels(
	...
	<ConfirmDialog
		...
		on:confirm={() => {
			...
			reader.onload = async (event: ProgressEvent<FileReader>) => {
				...
				const importedFunctions = JSON.parse(result) as FunctionImportPayload[];
				console.log(importedFunctions);

				for (let func of importedFunctions) {
					if ('function' in func) {
