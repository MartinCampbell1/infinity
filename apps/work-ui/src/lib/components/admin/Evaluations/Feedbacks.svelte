<script lang="ts">
	...
	type FeedbackSortKey = 'updated_at' | 'user' | 'model_id' | 'rating';

	const getFeedbackRating = (feedback: FeedbackItem): string | null =>
		feedback.data?.rating != null ? String(feedback.data.rating) : null;

	let page = 1;
	let items: FeedbackItem[] | null = null;
	let total: number | null = null;

	let orderBy: FeedbackSortKey = 'updated_at';
	let direction: 'asc' | 'desc' = 'desc';

	const setSortKey = (key: FeedbackSortKey) => {
		if (orderBy === key) {
						...
						>
							<td class=" py-0.5 text-right font-medium">
								<div class="flex justify-center">
									<Tooltip content={feedback?.user?.name ?? undefined}>
										<div class="shrink-0">
											<img
								...
								<div class="flex flex-col items-start gap-0.5 h-full">
									<div class="flex flex-col h-full">
										{#if feedback.data?.sibling_model_ids}
											<Tooltip content={feedback.data?.model_id ?? undefined} placement="top-start">
												<div
							...
							<td class="px-3 py-1 text-right font-medium text-gray-900 dark:text-white w-max">
								<div class=" flex justify-end">
									{#if getFeedbackRating(feedback) === '1'}
										<Badge type="success" content={$i18n.t('Won')} />
									{:else if getFeedbackRating(feedback) === '0'}
										<Badge type="muted" content={$i18n.t('Draw')} />
									{:else if getFeedbackRating(feedback) === '-1'}
										<Badge type="danger" content={$i18n.t('Lost')} />
									{/if}
								</div>
							</td>
