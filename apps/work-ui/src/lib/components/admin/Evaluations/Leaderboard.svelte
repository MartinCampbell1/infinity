<script lang="ts">
	...
	type LeaderboardSortKey = 'rating' | 'name' | 'won' | 'lost';
	...
	] as const satisfies Array<{ key: LeaderboardSortKey; label: string; class: string }>;

	let rankedModels: LeaderboardRow[] = [];
	let query = '';
	let loading = true;
	let debounceTimer: ReturnType<typeof setTimeout> | null = null;
	let orderBy: LeaderboardSortKey = 'rating';
	...
	const isHiddenModel = (meta: unknown): boolean => {
		...
		return (meta as Record<string, unknown>).hidden === true;
	};
	...
	const openModal = (model: LeaderboardRow) => {
		selectedModel = model;
		showModal = true;
	};
	...
	const loadLeaderboard = async (searchQuery = '') => {
		...
		try {
			...
			rankedModels = $models
				.filter((model) => !isHiddenModel(model?.info?.meta))
				.map((model) => {
	...
	$: sortedModels = [...rankedModels].sort((a, b) => {
		const getValue = (m: LeaderboardRow, key: LeaderboardSortKey): string | number => {
		...
		if (orderBy === 'name') {
			const aVal = getValue(a, orderBy) as string;
			const bVal = getValue(b, orderBy) as string;
			return direction === 'asc' ? aVal.localeCompare(bVal) : bVal.localeCompare(aVal);
		}
		const aVal = Number(getValue(a, orderBy));
		const bVal = Number(getValue(b, orderBy));
		...
		>
			<thead class="text-xs text-gray-800 uppercase bg-transparent dark:text-gray-200">
				<tr class="border-b-[1.5px] border-gray-50 dark:border-gray-850/30">
					{#each leaderboardColumns as col}
						<th
							scope="col"
