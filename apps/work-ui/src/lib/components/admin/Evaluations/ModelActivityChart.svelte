<script lang="ts">
	...
	type ChartPoint = ActivityPoint | WeeklyActivityPoint;

	type ChartConstructor = new (
		item: HTMLCanvasElement,
		config: ChartConfiguration<'bar', number[], string>
	) => ChartInstance<'bar', number[], string>;

	export let history: ActivityPoint[] = [];
	export let loading = false;
	export let aggregateWeekly = false;

	let chartCanvas: HTMLCanvasElement | null = null;
	let chartInstance: ChartInstance<'bar', number[], string> | null = null;
	...
	const createChart = async () => {
		...
		if (!ChartCtor) {
			...
			ChartCtor = module.default as ChartConstructor;
		}
		...
		let chartData: ChartPoint[] = history;

		if (aggregateWeekly && history.length > 7) {
			// Aggregate daily data into weekly buckets
			const weeklyData: Record<string, WeeklyActivityPoint> = {};
			history.forEach((h) => {
					...
					y: {
						...
						ticks: {
							...
							callback: (value: string | number) => {
								return Math.abs(Number(value)); // Show absolute values on y-axis
							}
						},
						border: {
							display: false
		...
		chartInstance = new ChartCtor(chartCanvas, config);
	};

	$: if (chartCanvas && history.length && !loading) {
		createChart();
	}
