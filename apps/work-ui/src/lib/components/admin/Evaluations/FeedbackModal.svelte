<script lang="ts">
	...
	import type { FeedbackDetailsResponse, FeedbackItem, FeedbackSnapshotMessage } from './types';
				...
				{#if loaded}
					<div class="flex flex-col w-full">
						...
						{#if feedbackData}
							...
							{@const parentId = messages[messageId]?.parentId ?? ''}
							...
							{#if parentId && messages[parentId]}
								<div class="flex flex-col w-full mb-2">
									...
									<div class="flex-1 text-xs whitespace-pre-line break-words">
										<span>{messages[parentId]?.content || '-'}</span>
									</div>
								</div>
							{/if}
