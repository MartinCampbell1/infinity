<script lang="ts">
	...
	type DirectConnections = object | null;
	...
	let modelsImportInputElement: HTMLInputElement | null = null;

	let models: ModelRecord[] | null = null;

	let workspaceModels: ModelRecord[] | null = null;
	let baseModels: ModelRecord[] | null = null;
	...
	$: activeModel = models?.find((model) => model.id === selectedModelId) ?? null;
	...
	$: if (models) {
		filteredModels = models
			...
			.filter((model) => {
				if (viewOption === 'enabled') return model?.is_active ?? true;
				if (viewOption === 'disabled') return !(model?.is_active ?? true);
				if (viewOption === 'visible') return !(model?.meta?.hidden ?? false);
				if (viewOption === 'hidden') return model?.meta?.hidden === true;
				if (viewOption === 'public') return isPublicModel(model);
				if (viewOption === 'private') return !isPublicModel(model);
				return true; // All
			})
			.sort((left, right) => {
	...
	const enableAllHandler = async () => {
		const modelsToEnable = filteredModels.filter((model) => !(model.is_active ?? true));
		// Optimistic UI update
		modelsToEnable.forEach((model) => (model.is_active = true));
		models = models;
		// Sync with server
		await Promise.all(
	...
	const disableAllHandler = async () => {
		const modelsToDisable = filteredModels.filter((model) => model.is_active ?? true);
		// Optimistic UI update
		modelsToDisable.forEach((model) => (model.is_active = false));
		models = models;
		// Sync with server
		await Promise.all(
	...
	const showAllHandler = async () => {
		const modelsToShow = filteredModels.filter((model) => model?.meta?.hidden === true);
		// Optimistic UI update
	...
	const hideAllHandler = async () => {
		const modelsToHide = filteredModels.filter((model) => !(model?.meta?.hidden ?? false));
		// Optimistic UI update
	...
	const downloadModels = async (modelsToExport: ModelRecord[]) => {
		let blob = new Blob([JSON.stringify(modelsToExport)], {
										...
										try {
											...
											toast.error(
												e && typeof e === 'object' && 'detail' in e
													? `${(e as { detail?: unknown }).detail ?? $i18n.t('Invalid JSON file')}`
