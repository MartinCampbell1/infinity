<script lang="ts">
	...
	const getVoices = async () => {
		if (TTS_ENGINE === '') {
			...
			if (res) {
				console.log(res);
				const apiVoices = res.voices as Array<{
					id?: string;
					voiceURI?: string;
					name?: string;
				}>;

					voices = apiVoices.map((voice) => {
						return {
							voiceURI: voice.voiceURI ?? voice.id ?? '',
							name: voice.name ?? voice.voiceURI ?? voice.id ?? '',
							localService: false
						};
					});
				voices.sort((a, b) => a.name.localeCompare(b.name, $i18n.resolvedLanguage));
	...
	const updateConfigHandler = async () => {
		...
		const payload: AudioConfigPayload = {
			tts: {
				...
				<div class="mb-2 py-0.5 flex w-full justify-between">
					...
					<div class="flex items-center relative">
						<select
							...
							on:change={async (e) => {
								const nextEngine = (e.currentTarget as HTMLSelectElement).value;

								await updateConfigHandler();
								await getVoices();
								await getModels();

								if (nextEngine === 'openai') {
									TTS_VOICE = 'alloy';
									TTS_MODEL = 'tts-1';
								} else {
									TTS_VOICE = '';
