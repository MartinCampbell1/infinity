<script lang="ts">
	...
	type PipelineValvePropertySpec = {
		title?: string | null;
		type?: 'array' | 'boolean' | 'string' | 'number' | 'integer' | null;
		enum?: string[] | null;
		[key: string]: unknown;
	};

	type PipelineValvesSpec = {
		properties: Record<string, PipelineValvePropertySpec>;
	};

	type PipelineValveValue = string | boolean | null | string[];
	type PipelineValves = Record<string, PipelineValveValue>;
	...
	let pipelineUploadInputElement: HTMLInputElement | null = null;

	let PIPELINES_LIST: PipelineListEntry[] | null = null;
	let selectedPipelinesUrlIdx = '';

	let pipelines: PipelineRecord[] | null = null;
	let selectedPipelineIdx: number | null = null;
	let selectedPipeline: PipelineRecord | null = null;
	$: selectedPipeline =
		selectedPipelineIdx !== null && pipelines ? (pipelines[selectedPipelineIdx] ?? null) : null;

	let valves: PipelineValves | null = null;
	...
	const updateHandler = async () => {
		...
		if (pipeline && (pipeline?.valves ?? false) && valves && valves_spec) {
			const nextValves: PipelineValves = { ...valves };
			for (const property of Object.keys(valves_spec.properties)) {
				if (valves_spec.properties[property]?.type === 'array') {
					nextValves[property] = String(nextValves[property] ?? '')
						.split(',')
	...
	const getValves = async (idx: number) => {
		const pipeline = pipelines?.[idx] ?? null;
		if (!pipeline) {
			valves = null;
			valves_spec = null;
			return;
		...
		const normalizedValves: PipelineValves = { ...valves };
		for (const property of Object.keys(valves_spec.properties)) {
			if (valves_spec.properties[property]?.type === 'array') {
				const currentValue = normalizedValves[property];
				normalizedValves[property] = Array.isArray(currentValue)
					? currentValue.join(',')
					: String(currentValue ?? '');
			}
		}
	...
	const toggleCustomValve = (property: string) => {
		if (!valves) {
			return;
		}
	...
	const updateEnumValve = (property: string, value: string) => {
		if (!valves) {
			return;
		}
	...
	const updateBooleanValve = (property: string, value: boolean) => {
		if (!valves) {
			return;
		}
	...
	const updateTextValve = (property: string, value: string) => {
		if (!valves) {
			return;
		}
					...
					<div class="flex w-full">
						<div class="flex-1 mr-2">
							<input
								...
								bind:this={pipelineUploadInputElement}
							...
							/>
								...
								<button
									...
									type="button"
									on:click={() => {
										pipelineUploadInputElement?.click();
									}}
								>
								{#if pipelineFiles}
						...
						<div class="space-y-1">
							{#if pipelines.length > 0}
								<div class="flex gap-2">
									...
									<button
										class="px-2.5 bg-gray-100 hover:bg-gray-200 text-gray-800 dark:bg-gray-850 dark:hover:bg-gray-800 dark:text-gray-100 rounded-lg transition"
										aria-label={$i18n.t('Delete selected pipeline')}
										on:click={() => {
													...
													<div class="flex mt-0.5 mb-1.5 space-x-2">
														<div class=" flex-1">
															{#if valves_spec.properties[property]?.enum ?? null}
																<select
																	class="w-full rounded-lg py-2 px-4 text-sm bg-gray-50 dark:text-gray-300 dark:bg-gray-850 outline-hidden"
																	value={String(valves[property] ?? '')}
																	on:change={(event: Event) => {
																		const target = event.currentTarget as HTMLSelectElement | null;
																		if (target) {
																			updateEnumValve(property, target.value);
																		}
																	}}
																>
																	{#each valves_spec.properties[property].enum as option}
																		<option value={option} selected={option === String(valves[property] ?? '')}>
																			{option}
																		</option>
																	{/each}
																</select>
																...
																<div class="flex justify-between items-center">
																	...
																	<div class=" pr-2">
																		<Switch
																			...
																			on:change={() => {
																				updateBooleanValve(property, !(Boolean(valves?.[property])));
																			}}
																		/>
																	</div>
																</div>
															{:else}
																<input
																	...
																	on:input={(event: Event) => {
																		...
																		if (target) {
																			updateTextValve(property, target.value);
																		}
																	}}
																	autocomplete="off"
																	required
