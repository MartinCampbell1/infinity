<script lang="ts">
	...
	import type {
		...
		OllamaConnectionsConfig,
		OpenAIConnectionsConfig
	} from '$lib/components/connection-types';
	...
	let connectionsConfig: ConnectionSettingsFlags | null = null;
	...
	const addOpenAIConnectionHandler = async (connection: ConnectionForm) => {
	...
	const addOllamaConnectionHandler = async (connection: ConnectionForm) => {
	...
	onMount(async () => {
		if ($user?.role === 'admin') {
			let ollamaConfig: Partial<OllamaConnectionsConfig> = {};
			let openaiConfig: Partial<OpenAIConnectionsConfig> = {};
			...
			ENABLE_OPENAI_API = openaiConfig.ENABLE_OPENAI_API ?? null;
