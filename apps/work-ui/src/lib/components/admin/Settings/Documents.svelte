<script lang="ts">
	...
	type DocumentsContentExtractionEngine =
		| ''
		| 'external'
		| 'tika'
		| 'docling'
	...
	type DocumentsPdfLoaderMode = 'page' | 'single';
	type DocumentsMineruApiMode = 'local' | 'cloud';
	type DocumentsRerankingEngine = '' | 'external';
	type DocumentsOutputFormat = 'markdown' | 'json' | 'html';

	type DocumentsRAGConfig = {
		CONTENT_EXTRACTION_ENGINE: DocumentsContentExtractionEngine;
		PDF_EXTRACT_IMAGES: boolean;
		PDF_LOADER_MODE: DocumentsPdfLoaderMode;
		...
		DATALAB_MARKER_OUTPUT_FORMAT: DocumentsOutputFormat;
		...
		MINERU_API_MODE: DocumentsMineruApiMode;
		MINERU_API_URL: string;
		MINERU_API_KEY: string;
		MINERU_API_TIMEOUT: number | null;
		MINERU_PARAMS: string;
		...
		RAG_RERANKING_ENGINE: DocumentsRerankingEngine;
		RAG_RERANKING_MODEL: string;
		RAG_EXTERNAL_RERANKER_URL: string;
		RAG_EXTERNAL_RERANKER_API_KEY: string;
		TOP_K: number | null;
	...
	type DocumentsEmbeddingUpdatePayload = Parameters<typeof updateEmbeddingConfig>[1] & {
		ollama_config: {
			key: string;
			url: string;
		};
	...
	type DocumentsRAGUpdatePayload = Parameters<typeof updateRAGConfig>[1] & {
		[key: string]: unknown;
	};

	const createDefaultRAGConfig = (): DocumentsRAGConfig => ({
	...
	const normalizeRAGConfig = (config: unknown): DocumentsRAGConfig => {
		const source = (config ?? {}) as Record<string, unknown>;
		const next = { ...createDefaultRAGConfig(), ...source } as DocumentsRAGConfig;

		const allowedExtensions = source.ALLOWED_FILE_EXTENSIONS;
		next.ALLOWED_FILE_EXTENSIONS = Array.isArray(allowedExtensions)
			? allowedExtensions.map((ext: unknown) => String(ext)).join(', ')
			: typeof allowedExtensions === 'string'
				? allowedExtensions
				: '';

		next.DOCLING_PARAMS =
			typeof source.DOCLING_PARAMS === 'object' && source.DOCLING_PARAMS !== null
				? JSON.stringify(source.DOCLING_PARAMS, null, 2)
				: typeof source.DOCLING_PARAMS === 'string'
					? source.DOCLING_PARAMS
					: '';

		next.MINERU_PARAMS =
			typeof source.MINERU_PARAMS === 'object' && source.MINERU_PARAMS !== null
				? JSON.stringify(source.MINERU_PARAMS, null, 2)
				: typeof source.MINERU_PARAMS === 'string'
					? source.MINERU_PARAMS
					: '';

		next.FILE_MAX_SIZE = toNullableNumber(source.FILE_MAX_SIZE);
		next.FILE_MAX_COUNT = toNullableNumber(source.FILE_MAX_COUNT);
		next.FILE_IMAGE_COMPRESSION_WIDTH = toNullableNumber(source.FILE_IMAGE_COMPRESSION_WIDTH);
		next.FILE_IMAGE_COMPRESSION_HEIGHT = toNullableNumber(source.FILE_IMAGE_COMPRESSION_HEIGHT);
		next.CHUNK_SIZE = toNullableNumber(source.CHUNK_SIZE);
		next.CHUNK_OVERLAP = toNullableNumber(source.CHUNK_OVERLAP);
		next.CHUNK_MIN_SIZE_TARGET = toNullableNumber(source.CHUNK_MIN_SIZE_TARGET);
		next.TOP_K = toNullableNumber(source.TOP_K);
		next.TOP_K_RERANKER = toNullableNumber(source.TOP_K_RERANKER);
		next.RELEVANCE_THRESHOLD = toNullableNumber(source.RELEVANCE_THRESHOLD);
		next.HYBRID_BM25_WEIGHT = toNullableNumber(source.HYBRID_BM25_WEIGHT);
		next.MINERU_API_TIMEOUT = toNullableNumber(source.MINERU_API_TIMEOUT);
	...
	const embeddingModelUpdateHandler = async () => {
		...
		const payload: DocumentsEmbeddingUpdatePayload = {
	...
	const submitHandler = async () => {
		...
		const payload: DocumentsRAGUpdatePayload = {
			...RAGConfig,
