<script lang="ts">
	...
	type ImageGenerationEngine = 'openai' | 'comfyui' | 'automatic1111' | 'gemini' | '';
	type ImageEditEngine = 'openai' | 'comfyui' | 'gemini' | '';
	type GeminiEndpointMethod = 'predict' | 'generateContent' | '';
	...
	interface ImageSettingsConfig {
		...
		COMFYUI_WORKFLOW_NODES: WorkflowNodeConfig[];
		IMAGES_EDIT_COMFYUI_WORKFLOW: string;
		IMAGES_EDIT_COMFYUI_WORKFLOW_NODES: WorkflowNodeConfig[];
		IMAGES_GEMINI_API_BASE_URL: string;
		IMAGES_GEMINI_API_KEY: string;
		IMAGES_GEMINI_ENDPOINT_METHOD: GeminiEndpointMethod;
		ENABLE_IMAGE_EDIT: boolean;
		IMAGE_EDIT_MODEL: string;
		IMAGE_EDIT_SIZE: string;
		IMAGE_EDIT_ENGINE: ImageEditEngine;
	...
	let models: ImageGenerationModel[] | null = null;
	let config: ImageSettingsConfig | null = null;

	let showComfyUIWorkflowEditor = false;
	let REQUIRED_WORKFLOW_NODES: WorkflowNode[] = [
		{
			type: 'prompt',
			key: 'text',
			node_ids: ''
	...
	let REQUIRED_EDIT_WORKFLOW_NODES: WorkflowNode[] = [
		{
			type: 'image',
			key: 'image',
			node_ids: ''
	...
	const updateConfigHandler = async () => {
		...
		if (
			currentConfig.IMAGE_GENERATION_ENGINE === 'automatic1111' &&
			currentConfig.AUTOMATIC1111_BASE_URL === ''
		) {
			toast.error($i18n.t('AUTOMATIC1111 Base URL is required.'));
			currentConfig.ENABLE_IMAGE_GENERATION = false;

			return null;
		} else if (
			currentConfig.IMAGE_GENERATION_ENGINE === 'comfyui' &&
			currentConfig.COMFYUI_BASE_URL === ''
		) {
			...
			currentConfig.IMAGE_GENERATION_ENGINE === 'openai' &&
			currentConfig.IMAGES_OPENAI_API_KEY === ''
		) {
			...
			currentConfig.IMAGE_GENERATION_ENGINE === 'gemini' &&
			currentConfig.IMAGES_GEMINI_API_KEY === ''
		) {
		...
		const res = await updateConfig(localStorage.token, {
			...
			AUTOMATIC1111_PARAMS:
				...
				currentConfig.AUTOMATIC1111_PARAMS.trim() !== ''
					? JSON.parse(currentConfig.AUTOMATIC1111_PARAMS)
					: {},
			IMAGES_OPENAI_API_PARAMS:
				typeof currentConfig.IMAGES_OPENAI_API_PARAMS === 'string' &&
				currentConfig.IMAGES_OPENAI_API_PARAMS.trim() !== ''
					? JSON.parse(currentConfig.IMAGES_OPENAI_API_PARAMS)
					: {}
		}).catch((error) => {
			toast.error(`${error}`);
			return null;
	...
	const saveHandler = async () => {
		...
		if (currentConfig.COMFYUI_WORKFLOW) {
			if (!validateJSON(currentConfig.COMFYUI_WORKFLOW)) {
				toast.error($i18n.t('Invalid JSON format for ComfyUI Workflow.'));
				loading = false;
				return;
			}

			currentConfig.COMFYUI_WORKFLOW_NODES = REQUIRED_WORKFLOW_NODES.map((node) => {
				return {
					type: node.type,
					key: node.key,
					node_ids:
		...
		if (currentConfig.IMAGES_EDIT_COMFYUI_WORKFLOW) {
			if (!validateJSON(currentConfig.IMAGES_EDIT_COMFYUI_WORKFLOW)) {
				toast.error($i18n.t('Invalid JSON format for ComfyUI Edit Workflow.'));
				loading = false;
				return;
			}

			currentConfig.IMAGES_EDIT_COMFYUI_WORKFLOW_NODES = REQUIRED_EDIT_WORKFLOW_NODES.map((node) => {
				return {
					type: node.type,
					key: node.key,
					node_ids:
	...
	onMount(async () => {
		if ($user?.role === 'admin') {
			...
			if (res) {
				config = res as ImageSettingsConfig;
			}
			...
			if (currentConfig.ENABLE_IMAGE_GENERATION) {
				getModels();
			}

			if (currentConfig.COMFYUI_WORKFLOW) {
				try {
					currentConfig.COMFYUI_WORKFLOW = JSON.stringify(
						JSON.parse(currentConfig.COMFYUI_WORKFLOW),
						null,
						2
					);
				} catch (e) {
			...
			REQUIRED_WORKFLOW_NODES = REQUIRED_WORKFLOW_NODES.map((node) => {
				const n =
					currentConfig.COMFYUI_WORKFLOW_NODES.find((candidate) => candidate.type === node.type) ??
					node;
				return {
					type: n.type,
					key: n.key,
			...
			if (currentConfig.IMAGES_EDIT_COMFYUI_WORKFLOW) {
				try {
					currentConfig.IMAGES_EDIT_COMFYUI_WORKFLOW = JSON.stringify(
						JSON.parse(currentConfig.IMAGES_EDIT_COMFYUI_WORKFLOW),
						null,
						2
					);
				} catch (e) {
			...
			currentConfig.IMAGES_OPENAI_API_PARAMS =
				typeof res.IMAGES_OPENAI_API_PARAMS === 'string'
					? res.IMAGES_OPENAI_API_PARAMS
					: JSON.stringify(res.IMAGES_OPENAI_API_PARAMS ?? {}, null, 2);

			currentConfig.AUTOMATIC1111_PARAMS =
				typeof res.AUTOMATIC1111_PARAMS === 'string'
					? res.AUTOMATIC1111_PARAMS
					: JSON.stringify(res.AUTOMATIC1111_PARAMS ?? {}, null, 2);

			REQUIRED_EDIT_WORKFLOW_NODES = REQUIRED_EDIT_WORKFLOW_NODES.map((node) => {
				const n =
					currentConfig.IMAGES_EDIT_COMFYUI_WORKFLOW_NODES.find(
								...
								accept=".json"
									on:change={(e) => {
										...
										reader.onload = () => {
											...
											if (typeof result === 'string' && currentConfig) {
												currentConfig.COMFYUI_WORKFLOW = result;
											}
							...
							<div class="flex w-full justify-between items-center">
								...
								<div class="mt-1 text-xs text-gray-400 dark:text-gray-500">
									<CodeEditorModal
										...
										onChange={(e: string) => {
											const currentConfig = config!;
											currentConfig.COMFYUI_WORKFLOW = e;
										}}
									onSave={() => {
										console.log('Saved');
									}}
								...
								accept=".json"
									on:change={(e) => {
										...
										reader.onload = () => {
											const result = reader.result;
											if (typeof result === 'string' && currentConfig) {
												currentConfig.IMAGES_EDIT_COMFYUI_WORKFLOW = result;
											}
							...
							<div class="flex w-full justify-between items-center">
								...
								<div class="mt-1 text-xs text-gray-400 dark:text-gray-500">
									<CodeEditorModal
										...
										onChange={(e: string) => {
											const currentConfig = config!;
											currentConfig.IMAGES_EDIT_COMFYUI_WORKFLOW = e;
										}}
									onSave={() => {
										console.log('Saved');
									}}
