<script lang="ts">
	...
	type UserSortKey = 'role' | 'name' | 'email' | 'last_active_at' | 'created_at';
	...
	let users: UserListItem[] | null = null;
	let total: number | null = null;

	let query = '';
	let searchDebounceTimer: ReturnType<typeof setTimeout> | undefined;
	let orderBy: UserSortKey = 'created_at'; // default sort key
	let direction: SortDirection = 'asc'; // default sort order
	...
	const setSortKey = (key: UserSortKey) => {
		if (orderBy === key) {
	...
	const handleDeleteConfirm = () => {
		...
		void deleteUserHandler(selectedUser.id);
	};
	...
	const licenseBanner: BannerType = {
		id: 'user-list-license-error',
		type: 'error',
		title: 'License Error',
		content:
...
{#if selectedUser}
	<ConfirmDialog bind:show={showDeleteConfirmDialog} on:confirm={handleDeleteConfirm} />
{/if}
...
{#if ($config?.license_metadata?.seats ?? null) !== null && total !== null && total > $config?.license_metadata?.seats}
				...
				{#each users as user (user.id)}
					<tr class="bg-white dark:bg-gray-900 dark:border-gray-850 text-xs">
						<td class="px-3 py-1 min-w-[7rem] w-28">
							...
							>
								<Badge
									type={user.role === 'admin' ? 'default' : user.role === 'user' ? 'success' : 'muted'}
