<script lang="ts">
	...
	type GroupModalTab = 'general' | 'permissions' | 'users';
	...
	export let onSubmit: (group: GroupSubmitPayload) => Promise<unknown> | unknown = async () => {};
	export let onDelete: () => Promise<unknown> | unknown = async () => {};

	export let show = false;
	export let edit = false;

	export let group: EditableGroup | null = null;
	export let defaultPermissions: PartialGroupPermissions = {};

	export let custom = true;

	export let tabs: GroupModalTab[] = ['general', 'permissions', 'users'];

	let selectedTab: GroupModalTab = 'general';
	let loading = false;
	...
	export let data: GroupSettingsData = {};

	export let permissions: GroupPermissions = mergeGroupPermissions();

	const submitHandler = async () => {
		loading = true;

		const nextGroup: GroupSubmitPayload = {
			name,
			description,
			data,
			permissions
	...
	const init = () => {
		if (group) {
			...
			permissions = mergeGroupPermissions(group.permissions);
			data = group.data ?? {};
		...
		data = {};
		userCount = 0;
		permissions = mergeGroupPermissions(permissions);
	};
