<script lang="ts">
	...
	type ModelAnalyticsEntry = {
		model_id: string;
		count: number;
		name: string;
	};

	type RawModelAnalyticsEntry = {
		model_id: string;
		count: number;
	};

	type UserAnalyticsEntry = {
		user_id: string;
		name?: string;
		email?: string;
		count: number;
	...
	type ChartPeriod = 'all' | 'month' | 'week' | 'hour' | 'year';

	const chartPeriodMap: Record<string, ChartPeriod> = {
		'24h': 'hour',
		'7d': 'week',
		'30d': 'month',
		'90d': 'year',
	...
	let modelStats: ModelAnalyticsEntry[] = [];
	let userStats: UserAnalyticsEntry[] = [];
	let dailyStats: DailyAnalyticsEntry[] = [];
	...
	let sortedModels: ModelAnalyticsEntry[] = [];
	let sortedUsers: UserAnalyticsEntry[] = [];
	...
	const loadDashboard = async () => {
		...
		try {
			...
			const [summaryRes, modelsRes, usersRes, dailyRes, tokensRes] = (await Promise.all([
				...
				{ users?: UserAnalyticsEntry[] } | null,
				{ data?: DailyAnalyticsEntry[] } | null,
				TokenUsageResponse | null
			];

			summary = { ...summary, ...(summaryRes ?? {}) };

			const modelsMap = new Map($models.map((m) => [m.id, m.name || m.id]));
			modelStats = (modelsRes?.models ?? []).map((entry: RawModelAnalyticsEntry) => ({
				...entry,
		...
		{@const chartColors = [
			...
			<div class="mb-4">
				...
				<ChartLine
					...
					colors={chartColors}
					height={200}
					period={chartPeriodMap[selectedPeriod] ?? 'week'}
				/>
			</div>
		{/if}
{/if}
