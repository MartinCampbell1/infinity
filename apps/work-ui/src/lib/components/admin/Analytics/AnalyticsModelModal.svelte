<script lang="ts">
	...
	const loadMoreChats = async () => {
		...
		try {
			...
			const chats = res?.chats ?? [];
			const newChats: ChatListItem[] = chats.map((c: any) => ({
				id: c.chat_id,
				title: c.first_message || 'No preview',
				updated_at: c.updated_at,
				user_id: c.user_id,
			...
			const existingIds = new Set(chatList.map((c: ChatListItem) => c.id));
			const uniqueNewChats = newChats.filter((c: ChatListItem) => !existingIds.has(c.id));
