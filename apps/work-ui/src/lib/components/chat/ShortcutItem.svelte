<script lang="ts">
	...
	type ShortcutItemData = NonNullable<(typeof shortcuts)[Shortcut]>;

	export let shortcut: ShortcutItemData;
	...
	onMount(async () => {
		...
		const navigatorWithKeyboard = navigator as Navigator & {
			keyboard?: {
				getLayoutMap: () => Promise<Map<string, string>>;
			};
		};

		if (navigatorWithKeyboard.keyboard && 'getLayoutMap' in navigatorWithKeyboard.keyboard) {
			try {
				keyboardLayoutMap = await navigatorWithKeyboard.keyboard.getLayoutMap();
			} catch (error) {
...
<div class="w-full flex justify-between">
	...
	<div class="flex-shrink-0 flex justify-end self-start h-full space-x-1 text-xs">
		{#each shortcut.keys.filter(
			(key: string) => !(key.toLowerCase() === 'delete' && shortcut.keys.includes('Backspace'))
		) as key}
			<div
