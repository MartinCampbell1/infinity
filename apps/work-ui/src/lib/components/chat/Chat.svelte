...
const HERMES_ONLY_CHAT = true;
	...
	const emittedFounderosToolLifecycleEvents = new Set<string>();
	...
	const getHermesToolLifecycleEventName = (toolEvent: Record<string, any>) => {
		...
		return $i18n.t('Tool activity');
	};

	const getHermesToolLifecycleEventId = (
		messageId: string,
		toolEvent: Record<string, any>,
		toolName: string
	) => {
		for (const key of ['id', 'event_id', 'execution_id', 'run_id']) {
			if (typeof toolEvent?.[key] === 'string' && toolEvent[key].trim()) {
				return toolEvent[key].trim();
			}
		}

		const command = `${toolEvent?.command ?? ''}`.trim();
		const preview = `${toolEvent?.preview ?? toolEvent?.message ?? toolEvent?.summary ?? ''}`
			.trim()
			.split(/\r?\n/, 1)[0];
		const context = preview || command || `${toolEvent?.event ?? toolEvent?.type ?? 'tool'}`;
		return `${messageId}:${toolName}:${context}`;
	};

	const emitFounderosToolLifecycle = (
		messageId: string,
		toolEvent: Record<string, any>,
		toolState: 'running' | 'done' | 'failed'
	) => {
		const toolName = getHermesToolLifecycleEventName(toolEvent);
		const eventId = getHermesToolLifecycleEventId(messageId, toolEvent, toolName);
		const dedupeKey = `${messageId}:${eventId}:${toolState}`;

		if (emittedFounderosToolLifecycleEvents.has(dedupeKey)) {
			return;
		}

		emittedFounderosToolLifecycleEvents.add(dedupeKey);

		if (toolState === 'running') {
			emitFounderosToolStarted({ toolName, eventId });
		} else {
			emitFounderosToolCompleted({
				toolName,
				eventId,
				status: toolState === 'failed' ? 'failed' : 'completed'
			});
		}
	};

	const emitFounderosApprovalLifecycle = (
		approvalId: string,
		summary: string,
		messageId: string
	) => {
		const normalizedApprovalId = `${approvalId ?? ''}`.trim() || messageId;
		const normalizedSummary = `${summary ?? ''}`.trim();
		const dedupeKey = `${messageId}:${normalizedApprovalId}`;

		if (!normalizedApprovalId || !normalizedSummary || emittedFounderosApprovalEvents.has(dedupeKey)) {
			return;
		}

		emittedFounderosApprovalEvents.add(dedupeKey);
		emitFounderosApprovalRequested({
	...
	const emitFounderosFailure = (
		...
		const dedupeKey = `${scope}:${messageId}:${normalizedCode}:${normalizedMessage}`;

		if (!normalizedMessage || emittedFounderosErrorEvents.has(dedupeKey)) {
			return;
		}

		emittedFounderosErrorEvents.add(dedupeKey);
		emitFounderosError({
	...
	const attachInlineApproval = (
		...
		emitFounderosApprovalLifecycle(
			data?.approvalId ?? messageId,
			`${message.hermesApproval.title}${message.hermesApproval.description ? `: ${message.hermesApproval.description}` : ''}`,
			messageId
		);
	...
	const scrollMessageIntoView = async (messageId: string | null | undefined) => {
		if (!messageId) {
			return false;
		}
	...
	const handleFounderosSessionFocus = async (detail: { section?: string } | null) => {
		if (detail?.section === 'approvals') {
			await scrollMessageIntoView(composerPendingApprovalMessageId);
			await focusChatComposer();
			return;
		}

		if (detail?.section === 'chat') {
			await focusChatComposer();
		}
	};

	const handleFounderosSessionRetry = async (detail: { retryMode?: string } | null) => {
		const latestAssistantMessageId = getCurrentBranchLatestAssistantMessageId(history);
		if (!latestAssistantMessageId) {
			await focusChatComposer();
			return;
		}

		const latestAssistantMessage = history.messages?.[latestAssistantMessageId];
		...
		const needsStop =
			...
			!!taskIds ||
			latestAssistantMessage.done === false;
		...
		await regenerateResponse(latestAssistantMessage);
		await focusChatComposer();
	};
	...
	const attachHermesStreamApproval = (
		...
		const approvalId = `${data?.approvalId ?? data?.id ?? streamId ?? responseMessageId}`.trim();
		const attached = attachInlineApproval(
			responseMessageId,
			{
				...approval,
				approvalId
	...
	const navigateHandler = async () => {
		...
		emittedFounderosToolLifecycleEvents.clear();
		emittedFounderosApprovalEvents.clear();
		emittedFounderosErrorEvents.clear();
	...
	const handleFounderosAccountSwitch = () => {
		// No-op for now. Account changes are owned by the host; the chat stays truthful
		// by only reacting to the workspace state it can observe locally.
	};
	...
	$: showEmbeddedRootSurface = $founderosLaunchContext.embedded && !chatIdProp;
	...
	onMount(() => {
		...
		const founderosHostActionCleanup = createFounderosHostActionRelay({
			onAccountSwitch: handleFounderosAccountSwitch,
			onSessionRetry: handleFounderosSessionRetry,
	...
	const initNewChat = async () => {
		...
		if ($page.url.pathname.includes('/c/')) {
			window.history.replaceState(history.state, '', buildFounderosRootHref($founderosLaunchContext));
		}
	...
	const consumeHermesSessionStream = async ({
		...
		try {
			...
			emitFounderosFailure(responseMessageId, 'hermes.stream', error?.message ?? `${error}`);
			await finalizeHermesInterruptedStream({
				responseMessageId,
				targetChatId,
	...
	const handleOpenAIError = async (error, responseMessage) => {
		...
		emitFounderosFailure(
			responseMessage.id,
			'openai.error',
			errorMessage || $i18n.t('Failed to process response.')
		);
	...
	const initChatHandler = async (history) => {
		...
		if (HERMES_ONLY_CHAT || !$temporaryChatEnabled) {
			...
			window.history.replaceState(history.state, '', buildFounderosChatHref(_chatId, $founderosLaunchContext));
...
<div
	class="{$founderosLaunchContext.embedded ? 'h-full max-h-full' : 'h-screen max-h-[100dvh]'} transition-width duration-200 ease-in-out {$showSidebar
		? '  md:max-w-[calc(100%-var(--sidebar-width))]'
		: ' '} w-full max-w-full flex flex-col"
	id="chat-container"
>
	{#if !loading || showEmbeddedRootSurface}
