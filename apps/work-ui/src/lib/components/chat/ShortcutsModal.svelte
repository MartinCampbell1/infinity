<script lang="ts">
	...
	type ShortcutDefinition = NonNullable<(typeof shortcuts)[Shortcut]>;
	type CategorizedShortcuts = Record<string, ShortcutDefinition[]>;
	...
	$: {
		...
		for (const shortcut of Object.values(shortcuts) as Array<ShortcutDefinition | undefined>) {
			if (!shortcut) {
				continue;
			}

			if (!shortcut.setting || settingsMap[shortcut.setting.id] === shortcut.setting.value) {
