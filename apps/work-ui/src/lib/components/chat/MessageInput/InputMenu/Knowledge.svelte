<script lang="ts">
	...
	type KnowledgeSelection =
		| ({ type: 'collection' } & KnowledgeBaseItem)
		| ({ type: 'file'; name?: string | null } & KnowledgeFileItem);

	export let onSelect: (selection: KnowledgeSelection) => void = () => {};

	const decodeLabel = (value?: string | null) => decodeString(value ?? '');

	let loaded = false;
	let selectedIdx = 0;

	let selectedItem: KnowledgeBaseItem | null = null;

	let selectedFileItemsPage = 1;

	let selectedFileItems: KnowledgeFileItem[] | null = null;
	...
	const getSelectedFileItemsPage = async () => {
		...
		if (res) {
			const typedRes = res as KnowledgeFilePage;
			if (!typedRes) {
				selectedFileItemsLoading = false;
				return res;
			}

			selectedFileItemsTotal = typedRes.total;
			...
			if (selectedFileItems) {
				...
				const newItems = pageItems.filter((item: KnowledgeFileItem) => !existingIds.has(item.id));
	...
	let page = 1;
	let items: KnowledgeBaseItem[] | null = null;
	...
	const getItemsPage = async () => {
		...
		if (res) {
			const typedRes = res as KnowledgeBasePage;
			if (!typedRes) {
				itemsLoading = false;
				return res;
			}
			...
			if (items) {
				...
				const newItems = pageItems.filter((item: KnowledgeBaseItem) => !existingIds.has(item.id));
				...
				>
					...
					>
						<div class="w-full text-left text-black dark:text-gray-100 flex items-center gap-1">
							...
							<Tooltip
								content={item.description || decodeLabel(item?.name)}
								placement="top-start"
								className="flex flex-1 min-w-0"
							>
								<div class="line-clamp-1 flex-1 text-sm">
									{decodeLabel(item?.name)}
								</div>
							</Tooltip>
						</div>
					</button>
							...
							{#each selectedFileItems as file, fileIdx (file.id)}
								...
								>
									<div class=" flex items-center gap-1.5">
										...
										<Tooltip content={decodeLabel(file?.meta?.name)} placement="top-start">
											<div class="line-clamp-1 flex-1 text-sm">
												{decodeLabel(file?.meta?.name)}
											</div>
										</Tooltip>
									</div>
								</button>
