<script lang="ts">
	...
	type AudioCacheEntry = HTMLAudioElement | true;
	type ChatStartDetail = { id: string };
	type ChatEventDetail = { id: string; content: string };
	type ChatFinishDetail = { id: string; content: string };
	type VideoInputChangeDetail = string;
	type KokoroDtype = 'fp32' | 'fp16' | 'q8' | 'q4' | 'q4f16';
	type AudioSettingsConfig = {
		stt?: {
			engine?: string;
			language?: string;
		};
	...
	export let submitPrompt: (prompt: string, options?: { _raw?: boolean }) => Promise<unknown> | unknown;
	export let stopResponse: () => void | Promise<void>;
	export let files: Array<{ type: string; url: string }> = [];
	...
	const audioConfig = () =>
		(($config as { audio?: AudioSettingsConfig } | undefined)?.audio ?? defaultAudioConfig);
	...
	const getModelTtsVoice = () =>
		((model?.info?.meta as { tts?: { voice?: string } } | undefined)?.tts?.voice ?? null);

	// Get voice: model-specific > user settings > config default
	const getVoiceId = () => {
		// Check for model-specific TTS voice first
		const modelTtsVoice = getModelTtsVoice();
		if (modelTtsVoice) {
			return modelTtsVoice;
		}
		// Fall back to user settings or config default
		if ($settings?.audio?.tts?.defaultVoice === audioConfig().tts?.voice) {
			return $settings?.audio?.tts?.voice ?? audioConfig().tts?.voice;
		}
		return audioConfig().tts?.voice;
	};
	...
	const audioCache = new Map<string, HTMLAudioElement | true>();
	const emojiCache = new Map<string, string>();

	const fetchAudio = async (content: string): Promise<AudioCacheEntry | undefined> => {
		if (!audioCache.has(content)) {
			try {
				...
				if ($settings.audio?.tts?.engine === 'browser-kokoro') {
					let ttsWorker = $TTSWorker as KokoroWorker | null;
					if (!ttsWorker) {
						ttsWorker = new KokoroWorker({
							dtype: $settings.audio?.tts?.engineConfig?.dtype ?? 'fp32'
						});
						TTSWorker.set(ttsWorker);
	...
	const chatStartHandler = async (e: Event) => {
		const { id } = (e as CustomEvent<ChatStartDetail>).detail;
	...
	const chatEventHandler = async (e: Event) => {
		const { id, content } = (e as CustomEvent<ChatEventDetail>).detail;
	...
	const chatFinishHandler = async (e: Event) => {
		const { id, content } = (e as CustomEvent<ChatFinishDetail>).detail;
	...
	<div class="max-w-lg w-full h-full max-h-[100dvh] flex flex-col justify-between p-3 md:p-6">
		...
		<div class="flex justify-between items-center pb-2 w-full">
			<div>
				{#if camera}
					<VideoInputMenu
						...
						on:change={async (e) => {
							const detail = (e as CustomEvent<VideoInputChangeDetail>).detail;
							console.log(detail);
							selectedVideoInputDeviceId = detail;
							await stopVideoStream();
							await startVideoStream();
						}}
					>
						<button
							...
							type="button"
							aria-label={$i18n.t('Switch video input')}
						>
							<svg
								xmlns="http://www.w3.org/2000/svg"
								viewBox="0 0 20 20"
			...
			<div>
				<button
					class=" p-3 rounded-full bg-gray-50 dark:bg-gray-900"
					aria-label={$i18n.t('Close call overlay')}
					on:click={async () => {
