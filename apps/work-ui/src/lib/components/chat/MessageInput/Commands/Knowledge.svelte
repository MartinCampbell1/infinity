<script lang="ts">
	...
	export let query = '';
	export let onSelect: (e: KnowledgeSelection) => void = () => {};

	let selectedIdx = 0;
	let items: KnowledgeItem[] = [];
	let searchDebounceTimer: ReturnType<typeof setTimeout> | undefined;

	export let filteredItems: KnowledgeItem[] = [];
	$: filteredItems = [
	...
	export const select = async () => {
		...
		const item = document.querySelector<HTMLElement>(`[data-selected="true"]`);
		if (item) {
			// click the item
			item.click();
		}
	...
	const getFolderItems = async () => {
		folderItems = ($folders as FolderStoreItem[])
			.map((folder) => ({
				...folder,
	...
	const getKnowledgeItems = async () => {
		...
		if (res) {
			knowledgeItems = res.items.map((item: KnowledgeBaseSearchItem) => {
				return {
					...item,
	...
	const getKnowledgeFileItems = async () => {
		...
		if (res) {
			fileItems = res.items.map((item: KnowledgeFileSearchItem) => {
				return {
					...item,
					type: 'file' as const,
					name: item.filename ?? '',
