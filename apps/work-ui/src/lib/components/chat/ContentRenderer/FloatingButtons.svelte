<script lang="ts">
	...
	type FloatingMessageRole = 'system' | 'user' | 'assistant' | 'tool';

	type FloatingMessage = {
		role: FloatingMessageRole;
		content: string;
	};
	...
	export let onAdd: (payload: FloatingAddPayload) => void = () => {};
	...
	const actionHandler = async (actionId: string | null | undefined) => {
		if (!actionId) {
			return;
		}
		...
		let selectedAction = actions.find((action) => action.id === actionId) ?? null;
		if (!selectedAction) {
					...
					for (const line of lines) {
						if (line.startsWith('data: ')) {
							if (line.startsWith('data: [DONE]')) {
								...
								try {
										const data = JSON.parse(line.slice(6)) as ChatCompletionDelta;
