<script lang="ts">
  ...
  export let item: ModelSelectorItem = {
    label: "",
    value: "",
    model: {} as Model,
  };
  ...
  const copyLinkHandler = async (model: Model) => {
  ...
  const getSortedTags = (model: ModelSelectorModel) =>
    [...(model.tags ?? [])].sort((a, b) => a.name.localeCompare(b.name));
  ...
  <div class="flex flex-col flex-1 gap-1.5">
    ...
    <div class="flex items-center gap-2">
      <div class="flex items-center min-w-fit">
        ...
        >
          <img
            ...
            alt={$i18n.t("{{modelName}} profile image", {
      ...
      <div class=" shrink-0 flex items-center gap-2">
            ...
            <div class="flex items-center translate-y-[0.5px] px-0.5">
              <Tooltip
                content={`${$i18n.t("Unloads {{FROM_NOW}}", {
                  FROM_NOW: dayjs(
                    item.model.ollama?.expires_at * 1000,
                  ).fromNow(),
                })}`}
                className="self-end"
              >
        ...
        {#if (item?.model?.tags ?? []).length > 0}
          {#key item.model.id}
            {@const tags = getSortedTags(item.model)}
