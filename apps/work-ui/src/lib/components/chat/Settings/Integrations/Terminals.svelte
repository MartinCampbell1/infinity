<script lang="ts">
	...
	export let servers: TerminalServerConnection[] = [];
	export let onChange: (servers: TerminalServerConnection[]) => void = () => {};

	let showAddModal = false;

	const normalizeServer = (
		server: Partial<TerminalServerConnection> & Pick<TerminalServerConnection, 'url'>
	...
	const addServer = (server: TerminalServerConnection) => {
		servers = [...servers, normalizeServer(server)];
		onChange(servers);
	};
	...
	const updateServer = (idx: number, updated: TerminalServerConnection) => {
		servers = servers.map((s, i) => (i === idx ? normalizeServer(updated) : s));
		onChange(servers);
	};
...
<AddTerminalServerModal
	...
	onSubmit={(server) =>
		addServer(normalizeServer(server as Partial<TerminalServerConnection> & Pick<TerminalServerConnection, 'url'>))}
/>
...
<div>
	...
	<div class="flex flex-col gap-1.5">
			{#each servers as server, idx}
				<Connection
					connection={normalizeServer(server)}
					onSubmit={(updated: TerminalServerConnection) => updateServer(idx, updated)}
