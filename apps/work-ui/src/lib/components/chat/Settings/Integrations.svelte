<script lang="ts">
	...
	type IntegrationsSettingsPatch = {
		toolServers?: ToolServerConnection[];
		terminalServers?: TerminalServerConfig[];
	};

	export let saveSettings: (settings: IntegrationsSettingsPatch) => void | Promise<void>;
	...
	const updateHandler = async () => {
		...
		const toolServersData = (await getToolServersData(servers)) as ToolServerResult[];
		const healthyToolServers = toolServersData.filter((data): data is ToolServerResult => Boolean(data));
		healthyToolServers.forEach((data) => {
			if (data.error) {
				toast.error(
		...
		toolServers.set(healthyToolServers.filter((data) => !data.error));
		...
		if (activeTerminals.length > 0) {
			const terminalServersData = (await getToolServersData(
			...
			terminalServers.set(terminalServersData.filter((data): data is ToolServerResult => Boolean(data) && !data.error).concat(existingSystemTerminals));
		} else {
	...
	onMount(() => {
		servers = Array.isArray($settings?.toolServers)
			? [...($settings.toolServers as ToolServerConnection[])]
			: [];
		terminalServerConfigs = Array.isArray($settings?.terminalServers)
			? ($settings.terminalServers as Array<Partial<TerminalServerConfig>>).map((server) => ({
					...server,
					name: server.name ?? '',
					url: server.url ?? '',
