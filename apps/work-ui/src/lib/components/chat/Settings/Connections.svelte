<script lang="ts">
	...
	export let saveSettings: (settings: { directConnections: DirectConnectionsConfig }) => void | Promise<void>;

	let config: DirectConnectionsConfig | null = null;

	let showConnectionModal = false;

	const addConnectionHandler = async (connection: ConnectionForm) => {
		if (!config) {
			return;
		}
							...
							{#each config?.OPENAI_API_BASE_URLS ?? [] as url, idx}
								<Connection
									...
									onDelete={() => {
										...
										currentConfig.OPENAI_API_BASE_URLS.forEach((url, newIdx) => {
											newConfig[newIdx] =
