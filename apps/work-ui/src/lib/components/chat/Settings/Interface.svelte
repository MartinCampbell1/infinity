<script lang="ts">
	...
	type ImageCompressionSize = NonNullable<Settings['imageCompressionSize']>;

	export let saveSettings: (patch: Partial<Settings>) => void | Promise<unknown>;
	...
	let floatingActionButtons: FloatingActionButtons = null;
	...
	const adjustTextScale = (delta: number) => {
		const nextTextScale = Math.max(1, Math.min(1.5, parseFloat(((textScale ?? 1) + delta).toFixed(2))));
		textScale = nextTextScale;
		setTextScaleHandler(nextTextScale);
	};
