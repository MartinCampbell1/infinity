<script lang="ts">
	...
	type FloatingActionButtons = FloatingActionButton[] | null;

	const createDefaultFloatingActionButtons = (): FloatingActionButton[] => [
		{
			id: 'ask',
			label: $i18n.t('Ask'),
			input: true,
	...
	export let onSave: (buttons: FloatingActionButtons) => void = () => {};

	export let floatingActionButtons: FloatingActionButtons = null;
	let workingFloatingActionButtons: FloatingActionButton[] = createDefaultFloatingActionButtons();
	let customFloatingActionButtonsEnabled = false;

	$: if (floatingActionButtons === null) {
		customFloatingActionButtonsEnabled = false;
		workingFloatingActionButtons = createDefaultFloatingActionButtons();
	} else {
		customFloatingActionButtonsEnabled = true;
		workingFloatingActionButtons = floatingActionButtons;
	}

	const submitHandler = async () => {
		onSave(customFloatingActionButtonsEnabled ? workingFloatingActionButtons : null);
		show = false;
	};
			...
			<div class=" flex flex-col w-full sm:flex-row sm:justify-center sm:space-x-6">
				...
				>
					<div>
						<div class="text-xs flex items-center justify-between mb-2">
							...
							<div class="flex items-center gap-2 text-gray-700 dark:text-gray-300">
								<button
									type="button"
									on:click={() => {
										if (!customFloatingActionButtonsEnabled) {
											workingFloatingActionButtons = createDefaultFloatingActionButtons();
											floatingActionButtons = workingFloatingActionButtons;
										} else {
								...
								{#if customFloatingActionButtonsEnabled}
									<button
										...
										on:click={() => {
											...
											while (workingFloatingActionButtons.some((b) => b.id === id)) {
												idx++;
												id = `new-button-${idx}`;
											}

											workingFloatingActionButtons = [
												...workingFloatingActionButtons,
												{
													id: id,
						...
						{#if !customFloatingActionButtonsEnabled || workingFloatingActionButtons.length === 0}
							<div class="text-gray-500 dark:text-gray-400 text-xs w-full text-center py-5">
								{$i18n.t('Default action buttons will be used.')}
							</div>
						{:else}
							{#each workingFloatingActionButtons as button, buttonIdx}
								<div class=" py-1 flex w-full justify-between items-start">
									...
									<button
										...
										on:click={() => {
											workingFloatingActionButtons = workingFloatingActionButtons.filter(
												(b) => b.id !== button.id
											);
											floatingActionButtons = workingFloatingActionButtons;
										}}
