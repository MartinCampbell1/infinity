<script lang="ts">
	...
	let TTSEngine = '';
	let TTSEngineConfig: TTSConfig = {};

	let TTSModel: KokoroModel | null = null;
	let TTSModelProgress: KokoroProgress | null = null;
	let TTSModelLoading = false;

	let voices: AudioVoice[] = [];
	let voice = '';
	...
	onMount(async () => {
		...
		if ($settings?.audio?.tts?.defaultVoice === audioConfig().tts?.voice) {
			voice = $settings?.audio?.tts?.voice ?? audioConfig().tts?.voice ?? '';
		} else {
	...
	const loadKokoro = async () => {
		if (TTSEngine === 'browser-kokoro') {
			...
			if (TTSEngineConfig?.dtype) {
				...
				const dtype = TTSEngineConfig.dtype || undefined;
				TTSModel = await KokoroTTS.from_pretrained(model_id, {
					dtype, // Options: "fp32", "fp16", "q8", "q4", "q4f16"
					device: hasWebGpu ? 'webgpu' : 'wasm', // Detect WebGPU
					progress_callback: (e) => {
						TTSModelProgress = e;
						console.log(e);
					}
		...
		saveSettings({
			audio: {
				...
				tts: {
					...
					defaultVoice: audioConfig().tts?.voice ?? '',
					nonLocalVoices: audioConfig().tts?.engine === '' ? nonLocalVoices : undefined
				}
			}
		});
		dispatch('save');
					...
					<div class=" mb-2.5 text-sm font-medium flex gap-2 items-center">
						...
						<div class=" text-sm font-medium shimmer">
							...
							{TTSModelProgress && TTSModelProgress.status === 'progress'
								? `(${Math.round((TTSModelProgress.progress ?? 0) * 10) / 10}%)`
								: ''}
						</div>
					</div>
