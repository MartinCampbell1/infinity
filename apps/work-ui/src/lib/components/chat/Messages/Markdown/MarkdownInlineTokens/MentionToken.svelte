<script lang="ts">
	...
	type MentionTokenData = Token & {
		id?: string;
		triggerChar?: string;
		label?: string;
	};

	export let token: MentionTokenData;

	let triggerChar = '';
	let label = '';

	let idType: 'U' | 'M' | 'C' | 'T' | null = null;
	let id = '';
	...
	const init = () => {
		...
		if (parts) {
			const nextIdType = parts[0];
			if (nextIdType === 'U' || nextIdType === 'M' || nextIdType === 'C' || nextIdType === 'T') {
				idType = nextIdType;
