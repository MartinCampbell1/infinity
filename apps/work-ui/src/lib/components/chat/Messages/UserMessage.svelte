<script lang="ts">
	...
	type UserMessageDirection = 'auto' | 'ltr' | 'rtl';

	type UserMessageFile = {
		url?: string | null;
	...
	type UserMessageRecord = {
		...
		user?: string | null;
		files?: UserMessageFile[] | null;
		[key: string]: unknown;
	};

	type UserMessageHistory = {
		messages: Record<string, UserMessageRecord>;
		currentId: string | null;
		[key: string]: unknown;
	};

	type UserMessageUser = {
		id?: string | null;
		name?: string | null;
		role?: string | null;
		permissions?: {
	...
	export let user: UserMessageUser | null = null;

	export let chatId: string;
	export let history: UserMessageHistory;
	...
	let editedFiles: UserMessageFile[] = [];

	let messageEditTextAreaElement: HTMLTextAreaElement;
	let editScrollContainer: HTMLDivElement;

	const normalizeChatDirection = (direction?: string | null): UserMessageDirection => {
		const normalized = (direction ?? 'auto').toLowerCase();
		if (normalized === 'ltr' || normalized === 'rtl' || normalized === 'auto') {
			return normalized;
		}
	...
	let message = structuredClone(history?.messages?.[messageId] ?? { id: messageId, childrenIds: [] }) as UserMessageRecord;
	$: if (history.messages) {
									...
									>
										<input
											...
											on:blur={(e: FocusEvent) => {
												...
												gotoMessage(message, Number(input.value) - 1);
