...
<script lang="ts">
	...
	$: displayModelName = message.modelName ?? model?.name ?? message.model;
	$: inheritedModelName =
	...
	$: activityMeta = [
		...
		.join(' • ');
	$: statusMeta =
		statusEntries.length > 1 ? $i18n.t('{{COUNT}} updates', { COUNT: statusEntries.length }) : '';
	$: toolsMeta =
		toolActivityCount > 1 ? $i18n.t('{{COUNT}} tools', { COUNT: toolActivityCount }) : '';
