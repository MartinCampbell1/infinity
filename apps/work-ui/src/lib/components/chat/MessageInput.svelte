...
<script lang="ts">
	...
	type ChatInputElement = {
		...
		getWordAtDocPos: () => string | undefined;
	...
	export let onUpload: (event: unknown) => void = () => {};
	export let onChange: (payload: OnChangePayload) => void = () => {};
	...
	export let history:
		| {
				...
				messages?: Record<string, { done?: boolean }>;
		  }
		| undefined;
	export let taskIds: string[] | null = null;

	export let prompt = '';
	export let files: ChatInputFile[] = [];
	...
	export let pendingOAuthTools: string[] = [];

	let showTerminalMenu = false;

	export let messageQueue: MessageQueueItem[] = [];
	export let onQueueSendNow: (id: string) => void = () => {};
	export let onQueueEdit: (id: string) => void = () => {};
	export let onQueueDelete: (id: string) => void = () => {};

	let inputContent: InputContent = null;

	let showInputVariablesModal = false;
	let inputVariablesModalCallback = (variableValues: Record<string, unknown>) => {};
	let inputVariables: Record<string, unknown> = {};
	let inputVariableValues: Record<string, unknown> = {};

	let showValvesModal = false;
	let selectedValvesType: 'tool' | 'function' = 'tool';
	...
	let suggestions: SuggestionConfig[] | null = null;
	...
	let chatInputContainerElement: HTMLElement | null = null;
	let chatInputElement: ChatInputElement | null = null;

	let filesInputElement: HTMLInputElement | null = null;
	let commandsElement: HTMLElement | null = null;
		...
		inputFiles.forEach(async (file) => {
			...
			if (file['type'].startsWith('image/')) {
				...
				reader.onload = async (event: ProgressEvent<FileReader>) => {
					let imageUrl = event.currentTarget?.result;
					...
					{#if autoScroll === false && history?.currentId}
						...
						>
								<button
									...
									aria-label={$i18n.t('Scroll to latest message')}
									type="button"
									on:click={() => {
										autoScroll = true;
										scrollToBottom();
