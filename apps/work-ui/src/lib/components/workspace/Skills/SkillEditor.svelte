<script lang="ts">
	...
	type SkillRecord = {
		...
		description?: string;
		content?: string;
		access_grants?: SkillAccessGrant[];
	};
	...
	export let onSubmit: (skill: SkillSubmitPayload) => Promise<void> | void;
	export let edit = false;
	export let skill: SkillRecord | null = null;
	...
	let accessGrants: SkillAccessGrant[] = [];
	...
	$: if (!edit && content) {
		const fm = parseFrontmatter(content) as FrontmatterResult;
		if (fm.name) {
	...
	onMount(async () => {
		if (skill) {
			...
			accessGrants = (skill?.access_grants ?? []) as SkillAccessGrant[];
