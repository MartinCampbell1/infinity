<script lang="ts">
	...
	type PromptRecord = {
		...
		content?: string;
		tags?: string[];
		access_grants?: PromptAccessGrant[];
	...
	export let onSubmit: (payload: PromptSubmitPayload) => Promise<unknown> | unknown;
	export let edit = false;
	export let prompt: PromptRecord | null = null;
	...
	let accessGrants: PromptAccessGrant[] = [];
	...
	let originalName = '';
	let originalCommand = '';
	let originalTags: PromptTag[] = [];
	let debounceTimer: ReturnType<typeof setTimeout> | null = null;

	let suggestionTags: PromptTag[] = [];
	...
	onMount(async () => {
		if (prompt) {
			const promptVersionId = prompt.version_id ?? null;
			name = prompt.name ?? '';
			await tick();
			const promptCommand = prompt.command ?? '';
			command = promptCommand.at(0) === '/' ? promptCommand.slice(1) : promptCommand;
			...
			if (edit) {
				...
				if (promptVersionId && history.length > 0) {
					selectedHistoryEntry = history.find((h) => h.id === promptVersionId) || history[0];
							...
							{#if selectedHistoryEntry.id === prompt?.version_id}
								...
								{:else}
									<button
										...
										on:click={() => {
											if (selectedHistoryEntry) {
												setAsProduction(selectedHistoryEntry);
											}
										}}
									>
								...
								<PromptHistoryMenu
									...
									onDelete={() => {
										if (selectedHistoryEntry) {
											handleDeleteHistory(selectedHistoryEntry.id);
										}
									}}
									onClose={() => {}}
								/>
