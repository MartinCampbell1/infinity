<script lang="ts">
	...
	const normalizePromptSuggestion = (value: PromptSuggestionInput | PromptSuggestion): PromptSuggestion => {
		...
		if (Array.isArray(value.title)) {
			return {
				title: [value.title[0] ?? '', value.title[1] ?? ''],
				content: value.content ?? ''
			};
		}
	...
	const setPromptSuggestions = () => {
		_promptSuggestions = promptSuggestions.map((s) => normalizePromptSuggestion(s));
	};
	...
	<div class="flex w-full justify-between mb-1.5">
		...
		<div class="flex justify-end gap-2">
				...
				on:change={(e) => {
					...
					reader.onload = () => {
						try {
							...
							const suggestions = (JSON.parse(result) as PromptSuggestionInput[]).map((s) =>
								normalizePromptSuggestion(s)
							);
			...
			<button
				...
				type="button"
				on:click={() => {
					const lastSuggestion = promptSuggestions.at(-1);
					if (!lastSuggestion || lastSuggestion.content !== '') {
			...
			{#each _promptSuggestions as prompt, promptIdx}
				...
				>
					...
					<button
						...
						on:click={() => {
							promptSuggestions = promptSuggestions.filter((_, index) => index !== promptIdx);
						}}
					>
						<svg
