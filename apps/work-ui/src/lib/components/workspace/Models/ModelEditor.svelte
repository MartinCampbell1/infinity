<script lang="ts">
	...
	type CapabilityKey = keyof typeof DEFAULT_CAPABILITIES;
	type ModelCapabilities = Record<CapabilityKey, boolean | undefined>;
	type DefaultFeatureId = 'web_search' | 'image_generation' | 'code_interpreter';

	type ModelBuiltinTools = Record<string, boolean>;

	type ModelTts = {
		voice?: string;
	};

	type ModelEditorParams = {
		system?: string | null;
		stream_response: boolean | null;
		stream_delta_chunk_size: number | null;
		function_calling: string | null;
		reasoning_tags: boolean | [string, string] | string[] | null;
		seed: number | null;
	...
	type ModelMeta = {
		...
		description: string | null;
		suggestion_prompts: ModelSuggestionPrompt[] | null;
		tags: ModelTag[];
		...
		actionIds?: string[];
		defaultFeatureIds?: DefaultFeatureId[];
	...
	type ModelEditorInfo = {
		...
		name: string;
		meta: ModelMeta;
		params: Partial<ModelEditorParams>;
	...
	type EditableModel = {
		...
		params?: Partial<ModelEditorParams> | null;
		access_grants?: ModelAccessGrant[] | null;
		preset?: boolean;
		arena?: boolean;
		direct?: boolean;
	...
	const DEFAULT_ADVANCED_PARAMS: ModelEditorParams = {
		system: '',
	...
	const SUPPORTED_PROFILE_IMAGE_TYPES: UploadableImageMime[] = [
		'image/gif',
		'image/webp',
		'image/jpeg',
		'image/png',
	...
	const createEmptyInfo = (): ModelEditorInfo => ({
		id: '',
		base_model_id: null,
		name: '',
		meta: {
	...
	const createEmptyCapabilities = (): ModelCapabilities => ({ ...DEFAULT_CAPABILITIES });
	...
	const getErrorMessage = (error: unknown) => {
		...
		if (error && typeof error === 'object' && 'detail' in error) {
			return `${(error as { detail?: unknown }).detail ?? error}`;
		}

		return `${error}`;
	};

	const isEligibleBaseModel = (candidate: EditableModel, currentModelId?: string) =>
		(currentModelId ? candidate.id !== currentModelId : true) &&
		!(candidate.preset ?? false) &&
		candidate.owned_by !== 'arena' &&
		!(candidate.direct ?? false);

	const normalizeStopValue = (value: ModelEditorParams['stop']) =>
		value
			? (typeof value === 'string' ? value.split(',') : value).filter((item) => item.trim())
			: null;

	const getAvailableFeatureIds = (capabilities: ModelCapabilities): DefaultFeatureId[] =>
		(Object.entries(capabilities) as Array<[CapabilityKey, boolean | undefined]>)
			.filter(
				([key, value]) =>
					Boolean(value) && ['web_search', 'code_interpreter', 'image_generation'].includes(key)
			)
			.map(([key]) => key as DefaultFeatureId);

	export let onSubmit: (info: ModelEditorInfo) => Promise<unknown> | unknown = async () => {};
	export let onBack: (() => void) | null = null;

	export let model: EditableModel | null = null;
	...
	let system = '';
	let info: ModelEditorInfo = createEmptyInfo();

	let params: ModelEditorParams = { ...DEFAULT_ADVANCED_PARAMS };
	...
	let capabilities: ModelCapabilities = createEmptyCapabilities();
	let defaultFeatureIds: DefaultFeatureId[] = [];
	let builtinTools: ModelBuiltinTools = {};

	let actionIds: string[] = [];
	let accessGrants: ModelAccessGrant[] = [];
	let tts: ModelTts = { voice: '' };

	let availableTools: ToolRecord[] = [];
	let toolOptions: SelectorItem[] = [];
	let availableFunctions: FunctionRecord[] = [];
	let filterFunctions: FunctionRecord[] = [];
	let filterOptions: SelectorItem[] = [];
	let actionFunctions: FunctionRecord[] = [];
	let actionOptions: SelectorItem[] = [];
	let toggleableFilterOptions: SelectorItem[] = [];
	let availableBaseModels: EditableModel[] = [];
	let availableFeatureIds: DefaultFeatureId[] = [];
	let descriptionValue = '';

	$: availableTools = Array.isArray($tools) ? ($tools as ToolRecord[]) : [];
	$: toolOptions = availableTools.map((tool) => ({
		id: tool.id,
	...
	$: availableFunctions = Array.isArray($functions) ? ($functions as FunctionRecord[]) : [];
	$: filterFunctions = availableFunctions.filter((func) => func.type === 'filter');
	$: filterOptions = filterFunctions.map((func) => ({
		id: func.id,
		name: typeof func.name === 'string' ? func.name : func.id,
		is_global: Boolean(func.is_global),
		meta: {
			description: typeof func.meta?.description === 'string' ? func.meta.description : undefined,
			toggle: func.meta?.toggle
		}
	}));
	$: actionFunctions = availableFunctions.filter((func) => func.type === 'action');
	$: actionOptions = actionFunctions.map((func) => ({
		id: func.id,
		name: typeof func.name === 'string' ? func.name : func.id,
		is_global: Boolean(func.is_global),
		meta: {
	...
	$: toggleableFilterOptions = filterOptions.filter(
		(func) =>
			(filterIds.includes(func.id) || Boolean(func.is_global)) && Boolean(func.meta?.toggle)
	);
	$: availableBaseModels = (($models ?? []) as EditableModel[]).filter((candidate) =>
		isEligibleBaseModel(candidate, model?.id)
	);
	$: availableFeatureIds = getAvailableFeatureIds(capabilities);
	$: info.meta.description = enableDescription ? descriptionValue : null;
	...
	const submitHandler = async () => {
		...
		const trimmedDescription = info.meta.description?.trim() ?? '';
		info.meta.description = enableDescription && trimmedDescription !== '' ? trimmedDescription : null;

		info.params.system = system.trim() === '' ? null : system;
		info.params.stop = normalizeStopValue(params.stop);
		(Object.keys(info.params) as Array<keyof ModelEditorParams>).forEach((key) => {
	...
	onMount(async () => {
		...
		defaultFeatureIds = (defaultMeta.defaultFeatureIds ?? []) as DefaultFeatureId[];
		builtinTools = (defaultMeta.builtinTools ?? {}) as ModelBuiltinTools;
		...
		if (model) {
			...
			id = model.id;
			descriptionValue = model?.meta?.description ?? '';

			enableDescription = model?.meta?.description !== null;

			let normalizedBaseModelId = model.base_model_id ?? null;
			if (model.base_model_id) {
				const baseModel = availableBaseModels.find((candidate) =>
					[model.base_model_id, `${model.base_model_id}:latest`].includes(candidate.id)
				);
				normalizedBaseModelId = baseModel?.id ?? null;
			}

			system = model?.params?.system ?? '';

			params = { ...DEFAULT_ADVANCED_PARAMS, ...(model?.params ?? {}) };
			...
			defaultFeatureIds = (model?.meta?.defaultFeatureIds as DefaultFeatureId[] | undefined) ?? defaultFeatureIds;
			builtinTools = (model?.meta?.builtinTools as ModelBuiltinTools | undefined) ?? builtinTools;
			...
			info = {
				...
				meta: {
					...createEmptyInfo().meta,
					...(JSON.parse(JSON.stringify(model?.meta ?? {})) as Partial<ModelMeta>)
				},
				params: {
					...JSON.parse(JSON.stringify(model?.params ?? {}))
				}
			};
		}
		...
		<input
			...
			on:change={() => {
				...
				reader.onload = (event: ProgressEvent<FileReader>) => {
					...
					const fileType = file.type as UploadableImageMime;
					...
					previewImage.onload = function () {
						...
						ctx.drawImage(previewImage, offsetX, offsetY, newWidth, newHeight);
				...
				if (SUPPORTED_PROFILE_IMAGE_TYPES.includes(file.type as UploadableImageMime)) {
		...
		{#if !edit || (edit && model)}
			...
			>
				<div class="w-full px-1">
								...
								<div class="mb-1">
									...
									<div>
										...
										>
											...
											>
											{#each availableBaseModels as baseModel}
					...
					<div class="my-4">
						<ToolsSelector bind:selectedToolIds={toolIds} tools={toolOptions} />
					</div>
					...
					{#if filterFunctions.length > 0 || actionFunctions.length > 0}
						...
						{#if filterFunctions.length > 0}
							...
							{#if toggleableFilterOptions.length > 0}
								<div class="my-4">
									<DefaultFiltersSelector
										bind:selectedFilterIds={defaultFilterIds}
										filters={toggleableFilterOptions}
									/>
								</div>
							{/if}
						{/if}
					...
					{#if availableFeatureIds.length > 0}
							<div class="my-4">
								<DefaultFeatures availableFeatures={availableFeatureIds} bind:featureIds={defaultFeatureIds} />
							</div>
					{/if}
