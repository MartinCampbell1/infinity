<script lang="ts">
	...
	type WorkspaceModelMeta = {
		hidden?: boolean;
		description?: string | null;
	};

	type WorkspaceModelUser = {
		name?: string | null;
		email?: string | null;
	};
	...
	type WorkspaceModel = {
		...
		meta?: WorkspaceModelMeta | null;
		user?: WorkspaceModelUser | null;
		[key: string]: unknown;
	};

	type WorkspaceModelImportEntry = Partial<WorkspaceModel> & {
		info?: WorkspaceModel;
	};

	type WorkspaceModelListResponse = {
	...
	const downloadModels = async (items: WorkspaceModel[] | null) => {
		const modelsToSave = items ?? [];
		let blob = new Blob([JSON.stringify(modelsToSave)], {
	...
	<div class="flex flex-col gap-1 px-1 mt-1.5 mb-3">
		<input
			...
			on:change={() => {
				...
				reader.onload = async (event: ProgressEvent<FileReader>) => {
					let savedModels: WorkspaceModelImportEntry[] = [];
					try {
						const fileReader = event.currentTarget as FileReader | null;
						...
						savedModels = JSON.parse(result) as WorkspaceModelImportEntry[];
		...
		<div class="flex justify-between items-center">
			...
			<div class="flex w-full justify-end gap-1.5">
				...
				{#if (total ?? 0) > 0 && ($user?.role === 'admin' || $user?.permissions?.workspace?.models_export)}
					<button
						class="flex text-xs items-center space-x-1 px-3 py-1.5 rounded-xl bg-gray-50 hover:bg-gray-100 dark:bg-gray-850 dark:hover:bg-gray-800 dark:text-gray-200 transition"
						on:click={async () => {
