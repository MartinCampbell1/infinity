<script lang="ts">
	...
	type ToolImportItem = Record<string, unknown>;

	let shiftKey = false;
	let loaded = false;

	let toolsImportInputElement: HTMLInputElement | null = null;
	...
	let selectedTool: ToolListItem | null = null;

	let showDeleteConfirm = false;

	let tools: ToolListItem[] = [];
	let filteredItems: ToolListItem[] = [];
	...
	const handleHorizontalScroll = (event: WheelEvent & { currentTarget: EventTarget & HTMLDivElement }) => {
		if (event.deltaY !== 0) {
	...
	const handleImportChange = () => {
		showConfirm = (importFiles?.length ?? 0) > 0;
	};
...
{#if loaded}
	...
	<DeleteConfirmDialog
		...
		on:confirm={() => {
			if (selectedTool) {
				void deleteHandler(selectedTool);
			}
		}}
	/>
	...
	<ConfirmDialog
		...
		on:confirm={() => {
			...
			reader.onload = async (event: ProgressEvent<FileReader>) => {
				...
				const importedTools = JSON.parse(result) as ToolImportItem[];
