<script lang="ts">
	...
	type ImportedSkillRecords = ImportedSkillRecord | ImportedSkillRecord[];
	...
	let filteredItems: SkillItem[] | null = null;
	...
	const handleImportChange = async (): Promise<void> => {
		...
		if (ext === 'json') {
			...
			reader.onload = async (event: ProgressEvent<FileReader>) => {
				try {
					...
					const parsedSkills = JSON.parse(content) as ImportedSkillRecords;
			...
			reader.onload = (event: ProgressEvent<FileReader>) => {
				...
				const fm = parseFrontmatter(mdContent) as { name?: string; description?: string };
...
{#if loaded}
	<div class="flex flex-col gap-1 px-1 mt-1.5 mb-3">
		<div class="flex justify-between items-center">
			...
			<div class="flex w-full justify-end gap-1.5">
				...
				{#if total !== null && ($user?.role === 'admin' || $user?.permissions?.workspace?.skills)}
					<button
						class="flex text-xs items-center space-x-1 px-3 py-1.5 rounded-xl bg-gray-50 hover:bg-gray-100 dark:bg-gray-850 dark:hover:bg-gray-800 dark:text-gray-200 transition"
						on:click={async () => {
	...
	<DeleteConfirmDialog
		...
		on:confirm={() => {
			if (selectedSkill) {
				void deleteHandler(selectedSkill);
			}
		}}
		message={
			selectedSkill
				? `${$i18n.t('This will delete')} ${selectedSkill.name}.`
				: $i18n.t('This will delete this skill.')
		}
	/>
{:else}
