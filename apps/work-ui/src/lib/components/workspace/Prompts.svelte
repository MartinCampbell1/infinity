<script lang="ts">
	...
	let promptsImportInputElement: HTMLInputElement | null = null;
	let loaded = false;
	...
	let prompts: PromptListItem[] | null = null;
	let tags: string[] = [];
	let total: number | null = null;
	let loading = false;

	let showDeleteConfirm = false;
	let deletePrompt: PromptListItem | null = null;
	...
	const getPromptList = async () => {
		...
		try {
			...
			if (res) {
				prompts = res.items as PromptListItem[];
	...
	const shareHandler = async (prompt: PromptListItem) => {
	...
	const cloneHandler = async (prompt: PromptListItem) => {
	...
	const exportHandler = async (prompt: PromptListItem) => {
		...
		<DeleteConfirmDialog
			...
			message={
				deletePrompt
					...
					: $i18n.t('This will delete this prompt.')
			}
			on:confirm={() => {
				if (deletePrompt) {
		...
		<input
			...
			on:change={() => {
				...
				reader.onload = async (event: ProgressEvent<FileReader>) => {
					...
					const savedPrompts = JSON.parse(result) as PromptImportItem[];
