<script lang="ts">
	...
	type UploadMenuAction = {
		type: 'files' | 'directory' | 'web' | 'text';
	};

	type AttachWebpagePayload = {
		type: 'web';
		data: string[];
	};
	...
	type DirectoryReaderLike = {
		readEntries: (
			success: (entries: UploadableEntry[]) => void,
	...
	type UploadableEntry = {
		...
		webkitGetAsEntry?: () => UploadableEntry | null;
	...
	let knowledge: KnowledgeRecord | null = null;
	let knowledgeId: string | null = null;

	let selectedFileId: string | null = null;
	let selectedFile: KnowledgeFileRecord | null = null;
	...
	const getItemsPage = async () => {
		if (knowledgeId === null || !knowledge) return null;
		...
		const res = (await searchKnowledgeFilesById(
			...
			return null;
		})) as KnowledgeSearchResponse | null;
	...
	const fileSelectHandler = async (file: KnowledgeFileRecord) => {
		try {
			selectedFile = file;
	...
	const uploadWeb = async (urls: string | string[]) => {
		...
		const newFileItems: KnowledgeFileRecord[] = urls.map((url: string) => ({
			type: 'file',
			file: '',
			id: null,
			url: url,
		...
		for (const fileItem of newFileItems) {
			try {
				...
				if (!fileUrl) {
					...
					toast.error($i18n.t('Failed to process URL: {{url}}', { url: fileItem.name ?? '' }));
					continue;
				}
	...
	const uploadFileHandler = async (file: File) => {
		...
		const maxFileSizeMb =
			($config as { file?: { max_size?: number } } | null)?.file?.max_size ?? null;
	...
	const handleModernBrowserUpload = async () => {
		const showDirectoryPicker = (
			window as Window & { showDirectoryPicker?: () => Promise<DirectoryHandleLike> }
		).showDirectoryPicker;
		...
		async function countFiles(dirHandle: DirectoryHandleLike) {
			for await (const entry of dirHandle.values()) {
				...
				if (entry.kind === 'file') {
					...
					if (!entry.name.startsWith('.')) {
						await countFiles(entry as DirectoryHandleLike);
					}
				}
			}
		}
		...
		async function processDirectory(dirHandle: DirectoryHandleLike, path = '') {
			for await (const entry of dirHandle.values()) {
				// Skip hidden files and directories
				if (entry.name.startsWith('.')) continue;
				...
				if (entry.kind === 'file') {
					...
					// Only process non-hidden directories
					if (!entry.name.startsWith('.')) {
						await processDirectory(entry as DirectoryHandleLike, entryPath);
					}
				}
			}
		}
	...
	const deleteFileHandler = async (fileId: string | null) => {
		if (!id || !fileId) {
			return;
		}
	...
	const onDrop = async (e: DragEvent) => {
		...
		const handleUploadingFileFolder = (items: Iterable<UploadableEntry>) => {
			for (const item of items) {
				...
				const isDirectory = wkentry?.isDirectory === true;
				if (isDirectory) {
					// Read the directory
					wkentry?.createReader?.().readEntries(
						(entries: UploadableEntry[]) => {
	...
	onMount(async () => {
		...
		const pageId = $page.params.id ?? null;
		if (!pageId) {
...
<AttachWebpageModal
	...
	onSubmit={async (e: AttachWebpagePayload) => {
		uploadWeb(e.data);
	}}
/>
			...
			<div class="px-3.5 flex flex-1 items-center w-full space-x-2 py-0.5 pb-2">
				<div class="flex flex-1 items-center">
					...
					{#if knowledge?.write_access}
						<div>
							<AddContentMenu
								onUpload={(data: UploadMenuAction) => {
