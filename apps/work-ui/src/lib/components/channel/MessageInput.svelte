<script lang="ts">
	...
	type MessageInputUser = {
		name: string;
	};

	type MessageInputChannel = {
		id: string;
	...
	type MessageInputFile = {
		...
		content_type?: string;
		status: 'uploading' | 'processed' | 'uploaded';
		size: number;
		error: string;
		itemId: string;
	};
	...
	type MessageInputSuggestion = unknown;
	type MessageInputKeyboardEvent = {
		event: KeyboardEvent;
	};
	type MessageInputClipboardEvent = {
		event: ClipboardEvent;
	};

	export let placeholder = $i18n.t('Type here...');
	export let chatInputElement: ChatInputElement = null;

	export let id: string | null = null;
	export let channel: MessageInputChannel = null;

	export let typingUsers: MessageInputUser[] = [];
	export let inputLoading = false;

	export let onSubmit: (e: { content: string; data: { files: MessageInputFile[] } }) => void = () => {};
	...
	export let replyToMessage: ReplyToMessage = null;
	...
	let suggestions: MessageInputSuggestion[] | null = null;

	const screenCaptureHandler = async () => {
		try {
			// Request screen media
			const mediaStream = await navigator.mediaDevices.getDisplayMedia({
				video: { cursor: 'never' } as MediaTrackConstraints & { cursor: 'never' },
				audio: false
			});
	...
	const inputFilesHandler = async (inputFiles: File[]) => {
		inputFiles.forEach(async (file) => {
			...
			const runtimeSettings = $settings as {
				...
				imageCompressionSize?: { width?: number | null; height?: number | null };
				audio?: { stt?: { language?: string } };
			};

			if (
				(runtimeConfig?.file?.max_size ?? null) !== null &&
				file.size > (runtimeConfig?.file?.max_size ?? 0) * 1024 * 1024
			) {
				console.error('File exceeds max size limit:', {
					fileSize: file.size,
					maxSize: (runtimeConfig?.file?.max_size ?? 0) * 1024 * 1024
				});
				toast.error(
					$i18n.t(`File size should not exceed {{maxSize}} MB.`, {
						maxSize: runtimeConfig?.file?.max_size
					})
				);
				return;
			}
			...
			if (file['type'].startsWith('image/')) {
				...
				reader.onload = async (event: ProgressEvent<FileReader>) => {
					const result = (event.currentTarget as FileReader | null)?.result;
					...
					imageUrl = await compressImageHandler(imageUrl, runtimeSettings, runtimeConfig);
				...
				reader.readAsDataURL(
					(file['type'] === 'image/heic' ? await convertHeicToJpeg(file) : file) as Blob
				);
			} else {
	...
	const uploadFileHandler = async (file: File, process = true) => {
		...
		const fileItem: MessageInputFile = {
			type: 'file',
			file: '',
			id: null,
			url: '',
		...
		suggestions = [
				...
				: []),
			{
				...
				render: getSuggestionRenderer(CommandSuggestionList, {
					i18n,
					onSelect: (e: { type?: string; data: any }) => {
						const { type, data } = e;
					...
					onUpload: (e: { type?: string; data: any }) => {
						const { type, data } = e;
...
{#if loaded}
	...
	<div class="bg-transparent">
		<div
			class="max-w-full mx-auto inset-x-0 relative"
			data-message-input-id={id ?? ''}
			data-show-formatting-toolbar={showFormattingToolbar}
		>
			<div
											...
											<div class=" relative group">
												...
												<div class=" absolute -top-1 -right-1">
													<button
														...
														type="button"
														aria-label={$i18n.t('Remove attachment')}
														on:click={() => {
									...
									{#key $settings?.richTextInput && $settings?.showFormattingToolbar}
										<RichTextInput
											...
											onChange={(e) => {
												...
												on:keydown={async (e: CustomEvent<MessageInputKeyboardEvent>) => {
												...
												on:paste={async (e: CustomEvent<MessageInputClipboardEvent>) => {
													const event = e.detail.event;
													console.log(event);
