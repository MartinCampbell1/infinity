<script lang="ts">
	...
	type ChannelRecord = {
		...
		is_manager?: boolean;
		access_grants?: ChannelGrant[] | null;
	...
	const isPublicChannel = (channel: ChannelRecord): boolean => {
				...
				<div class=" flex flex-col w-full sm:flex-row sm:justify-center sm:space-x-6">
					...
					>
						<div class="flex flex-col w-full h-full pb-2">
							<UserList
								channel={channelForUserList}
								onAdd={channelForUserList?.type === 'group' && channelForUserList?.is_manager
									? () => {
											showAddMembersModal = true;
										}
									: null}
								onRemove={channelForUserList?.type === 'group' && channelForUserList?.is_manager
									? (userId) => {
