<script lang="ts">
	...
	type RemoveHandler = ((userId: string) => void) | null;

	export let channel: ChannelRecord = null;

	export let onAdd: ActionHandler = null;
	export let onRemove: RemoveHandler = null;
	...
	let users: ChannelMember[] | null = null;
										...
										/>
											...
											<button
												...
												class=" rounded-full p-1 hover:bg-gray-100 dark:hover:bg-gray-850 transition disabled:opacity-50 disabled:cursor-not-allowed"
													...
													on:click={() => handleRemoveClick(user.id)}
												>
													<XMark />
												</button>
										</div>
