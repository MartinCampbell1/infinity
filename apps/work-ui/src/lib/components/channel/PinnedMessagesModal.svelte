	<script lang="ts">
		import { toast } from 'svelte-sonner';
		import { getContext, onMount } from 'svelte';
		import type { Writable } from 'svelte/store';
		import type { i18n as i18nType } from 'i18next';
		const i18n: Writable<i18nType> = getContext('i18n');
		import { resolveFounderosEmbeddedAccessToken } from '$lib/founderos/credentials';

	import { getChannelPinnedMessages, pinMessage } from '$lib/apis/channels';

	import Spinner from '$lib/components/common/Spinner.svelte';
	import Modal from '$lib/components/common/Modal.svelte';

	import XMark from '$lib/components/icons/XMark.svelte';
	import Message from './Messages/Message.svelte';
	import Loader from '../common/Loader.svelte';

	type ChannelRecord = {
		id: string;
		[key: string]: unknown;
	} | null;

	type PinnedMessage = {
		id: string;
		channel_id: string;
		is_pinned?: boolean;
		[key: string]: unknown;
	};

	type PinnedMessagesResponse = PinnedMessage[] | null;
	type PinHandler = (messageId: string, pinned: boolean) => void;

	export let show = false;
	export let channel: ChannelRecord = null;
	export let onPin: PinHandler = () => {};

	let page = 1;
	let pinnedMessages: PinnedMessage[] | null = null;

	let allItemsLoaded = false;
	let loading = false;
	const getWorkspaceAuthToken = () => resolveFounderosEmbeddedAccessToken();

	const getPinnedMessages = async () => {
		const currentChannel = channel;
		if (!currentChannel) return;
		if (allItemsLoaded) return;

		loading = true;
		try {
			const res = (await getChannelPinnedMessages(
				getWorkspaceAuthToken(),
				currentChannel.id,
				page
			).catch(
				(error) => {
					toast.error(`${error}`);
					return null;
				}
			)) as PinnedMessagesResponse;

			const nextMessages = Array.isArray(res) ? res : [];
			pinnedMessages = [...(pinnedMessages ?? []), ...nextMessages];

			if (nextMessages.length === 0) {
				allItemsLoaded = true;
			}
		} catch (error) {
			console.error('Error fetching pinned messages:', error);
		} finally {
			loading = false;
		}
	};

	const init = () => {
		page = 1;
		pinnedMessages = null;
		allItemsLoaded = false;

		getPinnedMessages();
	};

	const handlePin = async (message: PinnedMessage) => {
		pinnedMessages = (pinnedMessages ?? []).filter((m) => m.id !== message.id);
		onPin(message.id, !message.is_pinned);

		await pinMessage(
			getWorkspaceAuthToken(),
			message.channel_id,
			message.id,
			!message.is_pinned
		).catch((error) => {
			toast.error(`${error}`);
			return null;
		});

		init();
	};

	$: if (show) {
		init();
	}

	onMount(() => {
		init();
	});
</script>

{#if channel}
	<Modal size="sm" bind:show>
		<div>
			<div class=" flex justify-between dark:text-gray-100 px-5 pt-4 mb-1.5">
				<div class="self-center text-base">
					<div class="flex items-center gap-0.5 shrink-0">
						{$i18n.t('Pinned Messages')}
					</div>
				</div>
				<button
					class="self-center"
					on:click={() => {
						show = false;
					}}
				>
					<XMark className={'size-5'} />
				</button>
			</div>

			<div class="flex flex-col md:flex-row w-full px-4 pb-4 md:space-x-4 dark:text-gray-200">
				<div class=" flex flex-col w-full sm:flex-row sm:justify-center sm:space-x-6">
					<div class="flex flex-col w-full h-full pb-2 gap-1">
						{#if pinnedMessages === null}
							<div class="my-10">
								<Spinner className="size-5" />
							</div>
						{:else}
							<div
								class="flex flex-col gap-2 max-h-[60vh] overflow-y-auto scrollbar-thin scrollbar-thumb-gray-300 dark:scrollbar-thumb-gray-700 scrollbar-track-transparent py-2"
							>
								{#if pinnedMessages.length === 0}
									<div class=" text-center text-xs text-gray-500 dark:text-gray-400 py-6">
										{$i18n.t('No pinned messages')}
									</div>
								{:else}
									{#each pinnedMessages as message, messageIdx (message.id)}
										<Message
											className="rounded-xl px-2"
											{message}
											{channel}
											onPin={handlePin}
											onReaction={() => {}}
											onThread={() => {}}
											onReply={() => {}}
											onEdit={() => {}}
											onDelete={() => {}}
										/>

										{#if messageIdx === pinnedMessages.length - 1 && !allItemsLoaded}
											<Loader
												on:visible={() => {
													console.log('visible');
													if (!loading) {
														page += 1;
														getPinnedMessages();
													}
												}}
											>
												<div
													class="w-full flex justify-center py-1 text-xs animate-pulse items-center gap-2"
												>
													<Spinner className=" size-4" />
													<div class=" ">{$i18n.t('Loading...')}</div>
												</div>
											</Loader>
										{/if}
									{/each}
								{/if}
							</div>
						{/if}
					</div>
				</div>
			</div>
		</div>
	</Modal>
{/if}
