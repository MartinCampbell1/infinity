<script lang="ts">
	...
	type MessageDirection = 'auto' | 'ltr' | 'rtl';
	type ReactionUser = {
		id?: string | null;
		name?: string | null;
	};

	const normalizeChatDirection = (direction: string | null | undefined): MessageDirection => {
	...
	const handleReactionPickerSubmit = (...args: unknown[]) => {
	...
	const handleInlineReactionSubmit = (...args: unknown[]) => {
		const [name] = args as [string?];
		if (name) {
			onReaction(name);
		}
	...
	const handleEditedContentKeydown = (event: KeyboardEvent) => {
		if (event.key === 'Escape') {
	...
	const getReactionTooltipNames = (users: ReactionUser[]): string =>
		users
			.reduce((acc: string, userEntry, idx) => {
				const name = userEntry.id === $user?.id ? $i18n.t('You') : userEntry.name;
				const total = users.length;

				if (idx < 3) {
					const separator =
						idx === 0 ? '' : idx === Math.min(2, total - 1) ? ` ${$i18n.t('and')} ` : ', ';
				...
				>
						{#if onReaction}
							<EmojiPicker
								...
								onSubmit={handleReactionPickerSubmit}
							>
						...
						<div>
							<div class="flex items-center flex-wrap gap-y-1.5 gap-1 mt-1 mb-2">
									{#each message.reactions as reaction}
										<Tooltip
											content={$i18n.t('{{NAMES}} reacted with {{REACTION}}', {
												NAMES: getReactionTooltipNames(
													(reaction.users ?? []) as ReactionUser[]
												),
												REACTION: `:${reaction.name}:`
											})}
										>
											<button
												class="flex items-center gap-1.5 transition rounded-xl px-2 py-1 cursor-pointer {hasReactionFromCurrentUser(
													(reaction.users ?? []) as ReactionUser[]
												)
								...
								{/each}
									...
									{#if onReaction}
										<EmojiPicker
											onSubmit={handleInlineReactionSubmit}
										>
										<Tooltip content={$i18n.t('Add Reaction')}>
											<div
