...
import { folders, tags } from '$lib/stores';
	...
	type SearchItem = {
		id: string;
		name: string;
		type: 'tag' | 'folder' | 'pinned' | 'shared' | 'archived';
	};
	...
	const initTags = async () => {
		...
		await tags.set(await getAllTags(getWorkspaceAuthToken()));
		loading = false;
	};
	...
	const getSelectedIdx = () => selectedIdx ?? 0;
</script>
				...
				on:keydown={(e) => {
					...
					if (e.key === 'Enter') {
						if (filteredItems.length > 0) {
							const itemElement = document.getElementById(`search-item-${getSelectedIdx()}`);
						itemElement?.click();
						return;
					}

					if (filteredOptions.length > 0) {
						const optionElement = document.getElementById(`search-option-${getSelectedIdx()}`);
						optionElement?.click();
						return;
					}
				}
					...
					if (e.key === 'ArrowUp') {
						e.preventDefault();
						selectedIdx = Math.max(0, currentIdx - 1);
						...
						if (filteredItems.length > 0) {
							if (currentIdx === filteredItems.length - 1) {
								...
								selectedIdx = Math.min(currentIdx + 1, filteredItems.length - 1);
							}
						} else {
							if (currentIdx === filteredOptions.length - 1) {
								focused = false;
							} else {
								selectedIdx = Math.min(currentIdx + 1, filteredOptions.length - 1);
							}
						}
				} else {
