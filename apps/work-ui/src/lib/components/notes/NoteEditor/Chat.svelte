<script lang="ts">
	...
	import type { NoteFile, NoteMessage, NoteRecord, NoteSelection } from '../utils';
	...
	export let selectedContent: NoteSelection | null = null;

	export let files: NoteFile[] = [];
	export let messages: NoteMessage[] = [];

	export let onInsert: (content: string) => void = () => {};
	export let onStop: () => void = () => {};
	export let onEdited: () => void = () => {};

	export let insertNoteHandler: () => void = () => {};
	export let scrollToBottomHandler: () => void = () => {};
	export let inputElement: any = null;
...
`;
	...
	const chatCompletionHandler = async () => {
		...
		let responseMessage: NoteMessage;
		...
		const [res, controller] = await chatCompletion(
			localStorage.token,
			{
				model: model.id,
				stream: true,
				messages: chatMessages.map((m: NoteMessage) => ({
					role: m.role,
					content: m.content
				}))
	...
	const submitHandler = async (e: { content: string; data?: unknown }) => {
		const { content } = e;
					...
					>
						<div slot="menu" class="flex items-center justify-between gap-2 w-full pr-1">
							...
							<Tooltip content={selectedModelId}>
								...
								>
								{#each $models.filter((model) => !isModelHidden(model)) as model}
