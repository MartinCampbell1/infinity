<script lang="ts">
	...
	let importFiles = '';
	let selectedNote: Pick<NoteRecord, 'id' | 'title'> = {
		id: '',
		title: ''
	};
	...
	let items: NoteListItem[] | null = null;
	...
	let displayOption: 'grid' | null = null;
	let viewOption: 'created' | 'shared' | null = null;
	let permission: 'read_only' | null = null;
	...
	const groupNotes = (res: NoteListItem[] | null | undefined): [string, NoteListItem[]][] => {
		...
		return orderedKeys.map((key) => [key, grouped[key]] as [string, NoteListItem[]]);
	};
	...
	onMount(() => {
		viewOption = (localStorage?.noteViewOption ?? null) as typeof viewOption;
		displayOption = (localStorage?.noteDisplayOption ?? null) as typeof displayOption;
...
<div id="notes-container" class="w-full min-h-full h-full px-3 md:px-[18px]">
	{#if loaded}
		<DeleteConfirmDialog
			...
			message={`${$i18n.t('This will delete')} ${selectedNote.title}.`}
			on:confirm={() => {
			...
			<div class="px-3 flex justify-between">
				<div
					...
					on:wheel={(e) => {
						if (e.deltaY !== 0) {
							e.preventDefault();
							(e.currentTarget as HTMLElement | null)?.scrollBy({
								left: e.deltaY,
								behavior: 'auto'
							});
						}
