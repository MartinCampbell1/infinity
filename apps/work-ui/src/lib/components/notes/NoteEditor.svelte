<script lang="ts">
	...
	const hasPublicReadGrant = (grants: NoteAccessGrant[] | null | undefined) =>
	...
	let selectedContent: NoteSelection | null = null;
	...
	function areContentsEqual(a: unknown, b: unknown) {
		return JSON.stringify(a) === JSON.stringify(b);
	}

	function insertNoteVersion(noteToVersion: NoteRecord) {
		const current: NoteContent = {
			json: noteToVersion.data.content.json,
			html: noteToVersion.data.content.html,
			md: noteToVersion.data.content.md
		};
		const lastVersion = noteToVersion.data.versions?.at(-1);

		if (!lastVersion || !areContentsEqual(lastVersion, current)) {
			noteToVersion.data.versions = (noteToVersion.data.versions ?? []).concat(current);
			return true;
		}
		return false;
	}
...
${content}
	...
	async function enhanceNoteHandler() {
		...
		const model = $models
			.filter((model) => model.id === selectedModelId && !isModelHidden(model))
	...
	function setContentByVersion(versionIdx: number | null) {
	...
	// Navigation
	function versionNavigateHandler(direction: 'prev' | 'next') {
	...
	const inputFileHandler = async (file: File) => {
		...
		if (fileConfig?.file?.max_size !== null && file.size > (fileConfig?.file?.max_size ?? 0) * 1024 * 1024) {
			console.log('File exceeds max size limit:', {
				fileSize: file.size,
				maxSize: (fileConfig?.file?.max_size ?? 0) * 1024 * 1024
			});
			toast.error(
				$i18n.t(`File size should not exceed {{maxSize}} MB.`, {
					maxSize: fileConfig?.file?.max_size
				})
			);
			return;
		}
		...
		if (file['type'].startsWith('image/')) {
			const uploadImagePromise: Promise<NoteFile | null> = new Promise(async (resolve, reject) => {
				...
				const heicFile = file['type'] === 'image/heic' ? await convertHeicToJpeg(file) : file;
				reader.readAsDataURL(Array.isArray(heicFile) ? heicFile[0] : heicFile);
			});
	...
	const enhanceCompletionHandler = async (model: { id: string }) => {
		if (!note) {
			return;
		}
...
`;
											...
											.map((file) =>
												`${file.name}: ${
													typeof file.file === 'string'
														...
														: file.file?.data?.content ?? 'Could not extract content'
												}\n`
											)
											.join('')}</context>`
									: '')
	...
	const noteEventHandler = async (_note: NoteRecord) => {
		...
		for (const file of files) {
			if (file.type === 'image' || (file?.content_type ?? '').startsWith('image/')) {
				const e = new CustomEvent('data', { files: files } as any);
	...
	onMount(async () => {
		...
		if (!selectedModelId) {
			selectedModelId = $models.filter((model) => !isModelHidden(model)).at(0)?.id || '';
		}
...
{#if note}
	<AccessControlModal
		...
		bind:accessGrants={note.access_grants as any}
...
<DeleteConfirmDialog
	...
	message={`${$i18n.t('This will delete')} ${note.title}.`}
	on:confirm={() => {
						...
						<RichTextInput
							...
							onFileDrop={(currentEditor, files, pos) => {
								files.forEach(async (file: File) => {
									...
									if (!fileItem || fileItem.type !== 'image') {
										return;
									}

									const createdFileItem = fileItem;

									if (createdFileItem.type === 'image') {
							...
							onFilePaste={() => {}}
							on:paste={async (event: CustomEvent<{ event?: ClipboardEvent }> | ClipboardEvent) => {
								const e = (event as CustomEvent<{ event?: ClipboardEvent }>).detail?.event || event;
