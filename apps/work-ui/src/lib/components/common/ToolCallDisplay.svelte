<script lang="ts">
	...
	import { emitFounderosError, emitFounderosFileOpened } from '$lib/founderos/events';

	type ToolCallAttributeMap = {
		type?: string;
		id?: string;
		name?: string;
		arguments?: string;
	...
	type ToolCallFile = string | { type?: string; content_type?: string; url?: string };
	...
	export let attributes: ToolCallAttributeMap = {};
	...
	let lastToolSurfaceKey = '';
	let lastToolErrorKey = '';
	...
	function parseStringArray(str: string): string[] | null {
		const parsed = parseJSONString(str);
	...
	function parseToolCallFiles(str: string): ToolCallFile[] | null {
		const parsed = parseJSONString(str);
		if (
			Array.isArray(parsed) &&
			parsed.every((item) => typeof item === 'string' || isRecord(item))
		) {
			return parsed as ToolCallFile[];
		}
		return null;
	}

	function getRenderableToolSurfacePath() {
		if (!grouped && embeds && embeds.length > 0) {
			return embeds[0];
		}
		...
		if (files && files.length > 0) {
			for (const file of files) {
				...
				if (
					...
					file.url &&
					(file.type === 'image' || (file.content_type ?? '').toString().startsWith('image/'))
				) {
					return file.url;
				}
			}
	...
	function getMalformedToolFileIssue(): { signature: string; message: string } | null {
		if (!files || files.length === 0) {
			return null;
		}
		...
		for (const [index, file] of files.entries()) {
			...
			if (looksLikeImage && !file.url) {
				return {
					signature: `${componentId}:image-missing-url:${index}`,
					message: `Tool result image is missing a URL (${attributes.name ?? 'unknown tool'}).`
				};
			}
		}
	...
	$: files = parseToolCallFiles(decode(attributes?.files ?? ''));
	$: embeds = parseStringArray(decode(attributes?.embeds ?? ''));
	...
	$: {
		const nextSurfacePath = getRenderableToolSurfacePath();
		if (nextSurfacePath) {
			if (nextSurfacePath !== lastToolSurfaceKey) {
				lastToolSurfaceKey = nextSurfacePath;
				emitFounderosFileOpened({ path: nextSurfacePath });
			}
		} else {
			lastToolSurfaceKey = '';
		}

		const malformedIssue = getMalformedToolFileIssue();
		if (malformedIssue) {
			if (malformedIssue.signature !== lastToolErrorKey) {
				lastToolErrorKey = malformedIssue.signature;
				emitFounderosError({
					code: 'tool-call.file-missing-url',
					message: malformedIssue.message
				});
			}
		} else {
			lastToolErrorKey = '';
