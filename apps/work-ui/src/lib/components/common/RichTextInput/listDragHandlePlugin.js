...
export function listDragHandlePlugin(options = {}) {
	...
	function normalizeItemForList(state, itemNode, targetListNodeOrType) {
		...
		const wantedListTypeName =
			typeof targetListNode === 'string' ? targetListNode : targetListNode?.type?.name;
		if (!wantedListTypeName) return itemNode;

		const wantedListType = schema.nodes[wantedListTypeName];
	...
	function moveItem(view, fromStart, toPos) {
		...
		try {
			...
			if (wrapName) {
				...
				if (wrapType) {
					const frag = wrapType.create(null, Fragment.from([normalizeItemForList(state, orig, wrapType)]));
