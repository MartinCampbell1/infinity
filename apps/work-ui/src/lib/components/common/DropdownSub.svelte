<script lang="ts">
	...
	function positionContent(): void {
		...
		const parentContainer = triggerEl.closest('[class*="rounded"]')?.parentElement as
			| HTMLElement
			| null;
	...
	function handleMouseLeave(event: MouseEvent): void {
		...
		if (contentEl?.contains(relatedTarget)) return;
		if (triggerEl?.contains(relatedTarget)) return;
		open = false;
	}
