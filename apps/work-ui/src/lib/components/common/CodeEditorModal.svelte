<script lang="ts">
	...
	type CodeEditorModalChangeHandler = (value: string) => void | Promise<void>;
	type CodeEditorModalSaveHandler = () => void | Promise<void>;

	let {
		show = $bindable(),
		value = $bindable(),
		lang = 'python',
		onChange = (() => {}) as CodeEditorModalChangeHandler,
		onSave = (() => {}) as CodeEditorModalSaveHandler
	} = $props();

	let boilerplate = ``;

	let codeEditor = $state<InstanceType<typeof CodeEditor> | null>(null);
