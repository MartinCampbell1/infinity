<script lang="ts">
	...
	let systemTextareaElement: HTMLTextAreaElement | null = null;
	...
	let role: PlaygroundMessage['role'] = 'user';
	let message = '';
	...
	const chatCompletionHandler = async () => {
		...
		let responseMessage: PlaygroundMessage;
		if (messages.at(-1)?.role === 'assistant') {
			responseMessage = messages.at(-1) as PlaygroundMessage;
		} else {
			responseMessage = {
				role: 'assistant',
				content: ''
		...
		const textareaElement = document.getElementById(
			`assistant-${messages.length - 1}-textarea`
					...
					<div class="py-0.5">
						...
						<!-- $i18n.t('an assistant') -->
							<textarea
								...
								on:input={(e: Event & { currentTarget: HTMLTextAreaElement }) => {
									resizeTextarea(e.currentTarget, 150);
								}}
								on:focus={(e: Event & { currentTarget: HTMLTextAreaElement }) => {
									resizeTextarea(e.currentTarget, 150);
								}}
								rows="2"
							></textarea>
