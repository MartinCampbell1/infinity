...
from api.models import (
    ...
    get_cli_sessions, get_cli_session_messages, collapse_cli_session_clones,
    resolve_cli_canonical_session_id,
)
...
def handle_get(handler, parsed) -> bool:
    ...
    if parsed.path == '/api/session':
        ...
        requested_sid = sid
        cli_sessions = None
        if load_settings().get('show_cli_sessions'):
            try:
                cli_sessions = get_cli_sessions()
                sid = resolve_cli_canonical_session_id(sid, cli_sessions)
            except Exception:
                cli_sessions = None
        try:
            s = get_session(sid)
            ...
            try:
                if load_settings().get('show_cli_sessions'):
                    cli_map = {cs['session_id']: cs for cs in (cli_sessions or get_cli_sessions())}
                    if sid in cli_map:
                        sess['is_cli_session'] = True
            except Exception:
                pass
            if requested_sid != sid:
                sess['resolved_from_session_id'] = requested_sid
    ...
    if parsed.path == '/api/sessions':
        ...
        include_hidden_cli_clones = query.get('include_hidden_cli_clones', ['0'])[0] in ('1', 'true', 'yes')
        ...
        if show_cli_sessions:
            ...
            cli_map = {s['session_id']: s for s in cli}
            for ws in webui_sessions:
                if ws['session_id'] in cli_map:
                    cli_meta = cli_map[ws['session_id']]
                    ws['is_cli_session'] = True
                    ws['updated_at'] = max(ws.get('updated_at', 0) or 0, cli_meta.get('updated_at', 0) or 0)
                    ws['message_count'] = max(ws.get('message_count', 0) or 0, cli_meta.get('message_count', 0) or 0)
                    if ws.get('title') in ('', 'Untitled', 'CLI Session') and cli_meta.get('title'):
                        ws['title'] = cli_meta['title']
                    if ws.get('model') in ('', 'unknown', None) and cli_meta.get('model'):
                        ws['model'] = cli_meta['model']
                    for key in (
                        '_cli_first_user',
                        '_cli_last_user',
                        '_cli_generated_tail',
        ...
        if include_hidden_cli_clones:
            hidden_cli_clones = 0
        else:
            merged, hidden_cli_clones, _alias_map = collapse_cli_session_clones(merged)
        merged.sort(key=lambda s: s.get('updated_at', 0) or 0, reverse=True)
        cli_count = sum(1 for s in merged if s.get('is_cli_session'))
        diagnostics = {
            'active_profile': get_active_profile_name(),
            'active_hermes_home': str(get_active_hermes_home()),
            'show_cli_sessions': show_cli_sessions,
            'webui_session_count': len(webui_sessions),
            'raw_cli_session_count': len(cli),
            'merged_session_count': len(merged),
            'cli_count': cli_count,
            'hidden_cli_clones': hidden_cli_clones,
            'include_hidden_cli_clones': include_hidden_cli_clones,
        }
        return j(handler, {
            'sessions': merged,
            'cli_count': cli_count,
...
def _handle_session_import_cli(handler, body):
    ...
    requested_sid = sid
    sid = resolve_cli_canonical_session_id(sid)
    ...
    for cs in get_cli_sessions():
        if cs['session_id'] == sid:
            ...
            workspace = cs.get('workspace')
            title = cs.get('title') or title
            break

    # If already imported, refresh it from the current CLI transcript instead of
    # returning a stale local snapshot.
    ...
    return j(handler, {
        'session': s.compact() | {
            'messages': msgs,
            'is_cli_session': True,
            'resolved_from_session_id': requested_sid if requested_sid != sid else None,
        },
        'imported': imported,
        'refreshed': True,
    })
