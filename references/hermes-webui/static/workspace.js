...
function updateWorkspaceShell(){
  const title=$('workspacePanelTitle');
  const meta=$('workspacePanelMeta');
  const summary=$('workspaceShellSummary');
  if(!title||!meta||!summary)return;

  if(!S.session||!S.session.workspace){
    title.textContent='No workspace';
    title.title='';
    meta.textContent='Attach a workspace to inspect files and previews.';
    meta.title='';
  ...
  const name=parts[parts.length-1]||ws;
  const dirLabel=S.currentDir&&S.currentDir!=='.' ? S.currentDir : 'root';
  const entryCount=Array.isArray(S.entries)?S.entries.length:0;
  const countLabel=`${entryCount} item${entryCount===1?'':'s'}`;
  title.textContent=name;
  title.title=ws;
  meta.textContent=ws;
  meta.title=ws;
  summary.textContent=_previewCurrentPath ? `Preview · ${_previewCurrentPath}` : `${dirLabel} · ${countLabel}`;
}
