...
function updateQueueBadge(){
  ...
  if(MSG_QUEUE.length>0){
    ...
    const countText=MSG_QUEUE.length===1?'1 message queued':`${MSG_QUEUE.length} messages queued`;
    badge.innerHTML=`<span class="queue-badge-icon" aria-hidden="true"><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"/></svg></span><span class="queue-badge-copy"><span class="queue-badge-kicker">Queued</span><span class="queue-badge-text">${countText}</span></span>`;
  } else {
...
function showReconnectBanner(msg) {
  const text=$('reconnectMsgText');
  if(text) text.textContent = msg || 'A response may have been in progress when you last left.';
...
async function refreshSession() {
  ...
  if (!S.session) return;
  try {
    if(S.session.is_cli_session){
      try{
        await api('/api/session/import_cli',{method:'POST',body:JSON.stringify({session_id:S.session.session_id})});
      }catch(_){}
    }
...
let _cliRefreshTimer = null;
function startCliSessionAutoRefresh(){
  stopCliSessionAutoRefresh();
  _cliRefreshTimer=setInterval(async()=>{
    if(!S.session || !S.session.is_cli_session || S.busy) return;
    try{
      const synced=await api('/api/session/import_cli',{method:'POST',body:JSON.stringify({session_id:S.session.session_id})});
      const currentCount=(S.messages||[]).length;
      const nextCount=(synced.session?.messages||[]).length;
      const currentUpdated=S.session?.updated_at||0;
      const nextUpdated=synced.session?.updated_at||0;
      const nextSid=synced.session?.session_id||S.session.session_id;
      if(nextSid!==S.session.session_id || nextCount!==currentCount || nextUpdated!==currentUpdated){
        S.session=synced.session;
        localStorage.setItem('hermes-webui-session', S.session.session_id);
        S.messages=(synced.session.messages||[]).filter(m=>{
          if(!m||!m.role||m.role==='tool') return false;
          if(m.role==='assistant'){let c=m.content||'';if(Array.isArray(c))c=c.map(p=>p.text||'').join('');return String(c).trim().length>0;}
          return true;
        });
...
function stopCliSessionAutoRefresh(){
  if(_cliRefreshTimer){clearInterval(_cliRefreshTimer);_cliRefreshTimer=null;}
}
// ── Update banner ──
...
function syncTopbar(){
  if(!S.session){
    ...
    $('topbarMeta').textContent='Ready for code, planning, debugging, and file work';
    const modelChipLabel=$('modelChipLabel');
    if(modelChipLabel) modelChipLabel.textContent=getModelLabel($('modelSelect')?.value || 'openai/gpt-5.4-mini');
    if(typeof updateWorkspaceShell==='function') updateWorkspaceShell();
    return;
  }
  ...
  const workspaceLabel=(S.session.workspace||'').split('/').filter(Boolean).slice(-2).join('/') || 'No workspace';
  $('topbarMeta').textContent=`${workspaceLabel} · ${vis.length} entries`;
  ...
  const modelChipLabel=$('modelChipLabel');
  if(modelChipLabel) modelChipLabel.textContent=getModelLabel(displayModel);
...
function renderMessages(opts={}){
  ...
  const msgPane=$('messages');
  const wasPinned=forceBottom?true:_scrollPinned;
  const prevTop=msgPane?msgPane.scrollTop:0;
  ...
  for(let vi=0;vi<visWithIdx.length;vi++){
    ...
    const isCommentary=!isUser && m.phase==='commentary';
    ...
    const roleLabel=isUser?'You':(isCommentary?'Working':esc(_bn));
    const roleInitial=isUser?'Y':(isCommentary?'·':esc(_bn.charAt(0).toUpperCase()));
    row.classList.add(isUser?'user':'assistant');
    if(isCommentary) row.classList.add('commentary');
    row.innerHTML=`<div class="msg-head ${m.role}${isCommentary?' commentary':''}" ${tsTitle?`title="${esc(tsTitle)}"`:''}><div class="role-icon ${m.role}${isCommentary?' commentary':''}">${roleInitial}</div><span class="msg-label">${roleLabel}</span>${tsTitle?`<span class="msg-time">${new Date(tsVal*1000).toLocaleTimeString([],{hour:'2-digit',minute:'2-digit'})}</span>`:''}<span class="msg-actions">${editBtn}<button class="msg-copy-btn msg-action-btn" title="Copy" onclick="copyMsg(this)">&#128203;</button>${retryBtn}</span></div>${filesHtml}<div class="msg-body${isCommentary?' commentary':''}">${bodyHtml}</div>`;
...
function buildToolCard(tc){
  ...
  const runIndicator=isRunning?'<span class="tool-card-running-dot"></span>':'';
  const statusLabel=isRunning?'running':'done';
  ...
  row.innerHTML=`
    <div class="${cardClass}">
      <div class="tool-card-header" onclick="this.closest('.tool-card').classList.toggle('open')">
        ...
        <span class="tool-card-status ${isRunning?'running':'done'}">${statusLabel}</span>
...
function appendThinking(){
  ...
  const row=document.createElement('div');row.className='msg-row assistant commentary';row.id='thinkingRow';
  row.innerHTML=`<div class="msg-head assistant commentary"><div class="role-icon assistant commentary">·</div><span class="msg-label">Working</span></div><div class="thinking"><div class="dot"></div><div class="dot"></div><div class="dot"></div></div>`;
...
function renderBreadcrumb(){
  ...
  root.className='breadcrumb-seg breadcrumb-link breadcrumb-root';
  root.textContent=getWorkspaceShellRootLabel();
...
function getWorkspaceShellRootLabel(){
