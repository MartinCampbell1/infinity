...
(async()=>{
  ...
  // Restore last-used model preference, but do not let stale mini prefs override gpt-5.4
  const savedModel=localStorage.getItem('hermes-webui-model');
  if(savedModel && $('modelSelect')){
    const staleMini=savedModel==='openai/gpt-5.4-mini' || savedModel==='gpt-5.4-mini';
    if(staleMini){
      localStorage.setItem('hermes-webui-model','gpt-5.4');
